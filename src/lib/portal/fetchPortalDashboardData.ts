...
import type { MilestoneStatus, HealthStatus } from '@/components/portal'

export interface PortalDashboardTask {
  id: string
  title: string
...
export interface PortalDashboardMilestone {
  id: string
  name: string
...
export interface PortalDashboardActivity {
  id: string
...
export interface PortalDashboardApproval {
  id: string
  taskTitle: string
  approvedAt: string
  comment?: string
...
export interface PortalDashboardData {
  ...
  actionTasks: PortalDashboardTask[]
  ...
  milestones: PortalDashboardMilestone[]
  ballOwnership: {
    clientCount: number
    teamCount: number
  }
  ...
  approvals: PortalDashboardApproval[]
}

/**
 * AT-010 のクライアントダッシュボード集計ロジック本体。
 *
 * `/portal`（クライアント本人）と `/portal/preview/[spaceId]`（内部ユーザー向け
 * プレビュー）の両方から同一ロジックで呼ぶための単一ソース。呼び出し元は
 * spaceId の解決（クライアントは自分の space_memberships、プレビューは
 * org_memberships の役割チェック）だけを個別に行い、以降の集計はすべてここに
 * 委譲する — 将来ダッシュボードの集計ロジックだけが変わって「プレビューが嘘を
 * つく」事態を防ぐ。
 */
