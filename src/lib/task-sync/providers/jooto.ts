...
export const jootoAdapter: TaskSyncAdapter = {
  ...
  // 呼び出し回数の上限に収まる間隔にする。標準プランは**月100回**で、1サイクルの消費は
  // 1(ボード一覧) + 対象ボード数（差分APIが無く毎回全件取得するため）。
  //   間隔F分での月間消費 = (43200 / F) × (1 + ボード数)
  // 2日間隔(2880分)なら月15サイクル。ボード5つでも 15×6 = 90回で上限内に収まる。
  // 1日間隔だとボード3つで 30×4 = 120回となり、正常運用のまま上限を超えてしまう
  // （超えた時点でその月はまったく同期できなくなる＝一番avoidしたい壊れ方）。
  // ボードがさらに多い契約は接続設定 import_config.min_poll_interval_minutes で**延ばせる**
  // （縮める方向には効かない＝設定で上限超過＝同期停止を招かせない）。
  // ビジネスプランは呼び出し無制限だが、プランを機械的に判別する手段が無いため安全側に倒す。
  minPollIntervalMinutes: 2 * 24 * 60,
