...
export const jootoAdapter: TaskSyncAdapter = {
  ...
  // 1日1回に抑える。ただし**これだけでは足りない**: 1サイクルの消費は
  // 1(ボード一覧) + ボード数 なので、ボードが3つあれば 30×(1+3)=120回/月 で標準プランの
  // 上限(100回)を超える。対象ボード数はテナントごとに違いアダプタからは決められないため、
  // 接続設定 import_config.min_poll_interval_minutes で運用側が間隔を**延ばせる**ようにしてある
  // （縮める方向には効かない）。ビジネスプランは呼び出し無制限なのでこの制約は無い。
