  ...
  return {
    display_information: {
      ...
      long_description:
        ...
        'はじめ方は AgentPM の「AI秘書 → チャット連携 → Slack」の画面に沿ってください。画面に手順（全6つ）が出ます。\n\n' +
        // 同じワークスペースにツール連携の「AgentPM」も入るため、説明欄だけで役割の違いが分かるようにする
        '「AgentPM」との違い:\n' +
        'AgentPM でのタスク操作をチャンネルにお知らせしたり、/agentpm でタスクを作ったり、' +
        '@メンションで質問に答えたりするのは、別アプリ「AgentPM」（ツール連携）の役割です。' +
        'この秘書は、合言葉で紐づけたチャンネルだけの会話を読んで動きます。' +
        '両方を同じワークスペースに入れても競合しません。',
