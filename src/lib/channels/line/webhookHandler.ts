...
async function processPostback(
  ...
  // 期限リマインド確認ループ（設計正本 §7・PR-2）。確認Flexは1:1/グループどちらにも
  // 届き得る（§9宛先解決）ため、digest_doneのようなactiveGroup検証には乗せず、
  // 責任者確認postbackと同様アクター解決・テナント/認可を_via_line RPCに一任する専用
  // ハンドラへ委ねる。
  const dueReminderDoneAction = parseDueReminderDonePostback(data)
  const dueReminderSnoozeAction = dueReminderDoneAction ? null : parseDueReminderSnoozePostback(data)
  if (dueReminderDoneAction || dueReminderSnoozeAction) {
    await processDueReminderPostback(account, event, disabled, dueReminderDoneAction, dueReminderSnoozeAction)
    return
  }
...
/**
 * 期限リマインド確認ループ（設計正本 §7・PR-2）: [完了した] / [まだ][○日後に再通知] postback。
 *
 * digestの責任者確認（processApprovalPostback）と同型 — authz（口座×external_user_idからの
 * 内部ユーザー解決・org/space束縛）・single-winner遷移・監査・connector complete enqueueは
 * すべて rpc_confirm_task_done_via_line / rpc_snooze_due_reminder_via_line が1トランザクションで
 * 完結させるため、ここではwebhookで検証済みの (account.id, event.externalUserId) だけを渡し、
 * body由来の内部UUID（taskId/occurrenceId）以外は信頼しない。
 *
 * digest_done/undoと異なりactiveGroup検証はしない: 確認Flexは§9の宛先解決により1:1 DM・
 * 発生元グループ・spaceグループのいずれにも届き得るため、グループ世代の整合はRPC側の
 * テナント/space束縛に一任する。二重押下はRPCの冪等（2回目 already_done 等）で吸収される
 * 前提でそのまま呼ぶ。
 *
 * code review #4是正: 送信側(due-reminder-sender)はentitlementのみでゲートし、共有bot
 * （owner_type='platform'）配下のグループにもFlexを送る（Proの `line_direct_dm` を持たない
 * 組織・既存の共有グループがそのままPro化した組織 等）。よってここでplatformを一律dropすると
 * 共有bot経由で届いたボタンが無反応になる。RPCのauthz/監査はtask由来のorg（v_task.org_id）で
 * 完結し owner_type に依存しないため、ここでも platform を弾かない（他のpostback種別の
 * platform扱いは変えない）。
 *
 * 監査行(channel_messages)のorgIdはaccount.orgId（platformでは常にnull）ではなく、
 * グループ文脈ならそのグループが属するorg（digest_done/undoと同じ解決順序＝
 * findActiveGroup(account.id, event.groupId)）から取る。DM文脈（groupId無し）はaccount.orgId
 * を使う（DM許可はPro=自社LINE専有機能のため通常nullにならない）。org解決不能でも、RPCの
 * 主作用（完了/スヌーズ）は既に確定しているため返信自体は行う（監査行だけ諦める）。
 *
 * code review #5是正: forbidden/not_found/already_snoozed（世代不一致=旧世代Flexへの
 * 正当な無操作）はprocessApprovalPostbackと同方針で完全沈黙（返信も監査行も残さない）。
 * done/already_done/blocked/snoozed/cappedのみ、担当者への正当なフィードバックとして返信する。
 */
async function processDueReminderPostback(
  account: LineAccount,
  event: NormalizedLineEvent,
  disabled: boolean,
  doneAction: { taskId: string } | null,
  snoozeAction: { occurrenceId: string; days: number; expectedSendCount: number } | null,
): Promise<void> {
  const externalUserId = event.externalUserId
  if (!externalUserId) return

  const actionKind = doneAction ? 'due_reminder_done' : 'due_reminder_snooze'
  let result: string
  let replyText: string

  if (doneAction) {
    let status: 'done' | 'already_done' | 'forbidden' | 'blocked'
    try {
      const r = await confirmTaskDoneViaLine(account.id, externalUserId, doneAction.taskId)
      status = r.status
    } catch (e) {
      // 一過性のRPC失敗。バッチは落とさず、このイベントだけ何も残さず終える
      // （再タップで自然に回復する。承認postbackと同じ扱い）。
      console.error('LINE webhook: due reminder done postback RPC failed', doneAction.taskId, e)
      return
    }

    if (status === 'forbidden') return // code review #5: 完全沈黙（返信も監査行も残さない）
    result = status

    if (status === 'done') {
      // RPCはstatusのみを返す契約のため、返信文言用のタイトルは別途読み取る
      // （client供給のタイトルは信頼しない・ベストエフォート）。
      let title: string | null = null
      try {
        const task = await findTaskSnapshotForReminder(doneAction.taskId)
    ...
    let status: 'snoozed' | 'capped' | 'forbidden' | 'not_found' | 'already_snoozed'
    try {
      const r = await snoozeDueReminderViaLine(
        account.id,
        externalUserId,
        occurrenceId,
        days,
      ...
      console.error('LINE webhook: due reminder snooze postback RPC failed', occurrenceId, e)
      return
    }

    // code review #5: forbidden/not_found/already_snoozed（世代不一致=旧世代Flexへの正当な
    // 無操作）はすべて完全沈黙。グループへのノイズ返信と存在オラクル化の両方を避ける。
  ...
  // code review #4: account.orgIdはplatformで常にnullのため、グループ文脈では
  // digest_done/undoと同じ解決順序（グループが属するorg）を使う。
  const orgId = event.groupId ? ((await findActiveGroup(account.id, event.groupId))?.orgId ?? null) : account.orgId

  if (!orgId) {
    // 監査行は残せないが、RPCの主作用は既に確定しているため返信は行う。
    console.error('LINE webhook: due reminder postback org resolution failed', actionKind)
    if (disabled || !event.replyToken) return
    await replyLineMessage({
      accessToken: account.accessToken,
      replyToken: event.replyToken,
  ...
  const recorded = await insertChannelMessage({
    ...
    body: null,
    payload: doneAction
      ? { event: 'postback', action: actionKind, taskId: doneAction.taskId, result }
      : {
          event: 'postback',
          action: actionKind,
          occurrenceId: snoozeAction!.occurrenceId,
          days: snoozeAction!.days,
          result,
        },
    storagePath: null,
    status: 'received',
