...
export function buildAgentpmSkill(manifest: Manifest): string {
  const lines = [
    '---',
    'name: agentpm',
    'description: AgentPM（ボールで「次に動く人」を管理するタスク管理）を agentpm CLI で操作する。「タスクを作って」「進捗を確認して」「ボールを渡して」「レビュー」「マイルストーン」「会議・議事録」「Wiki に転記して」「CSV から取り込んで」「ファイルを上げて」などと言われたときに使う。',
    '---',
    '',
    '# AgentPM CLI（agentpm）',
    '',
    ...
    '',
    '- `権限エラー: Action "write" not allowed for this API key` など: 鍵に許可されていない操作。ユーザーに、設定画面で必要な操作（書き込み・一括操作など）を許可した鍵を発行し直してもらう。',
