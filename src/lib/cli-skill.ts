...
export function buildAgentpmSkill(manifest: Manifest): string {
  const lines = [
    ...
    '```',
    '',
    '## 決めることを札にして、決まったら確定する',
    '',
    '会議で決めることは「決める札」にする。ふつうのタスクとの違いは、**決まるまで完了にできない**こと。',
    '作り方は、Wiki ページに「仕様書として扱う」を入れて、そのページをタスクに紐づける。',
    '',
    '```',
    '# 決める札を作る（wikiPageId を渡すと決める札になる）',
    ...
    '```',
    '',
    '- 確定すると、紐づく Wiki ページの**末尾に**「決定: <札の題名> (日付)」が入る。書き足す前の本文は「確定時点の控え」として残る。',
    ...
    '',
    '```',
    '# 1) 議事録に「決めること」を書く（各行に Wiki ページのリンクを入れる）',
    ...
    '- 差し込んだページが「仕様書として扱う」なら決める札、そうでなければ参考資料付きのふつうのタスク。',
    '- 作成済みの行は飛ばす（同じ行から二重に作らない）。',
    '- **`--dry-run` で先に確かめる。** 行の書き方を間違えていると、意図しない札ができる。',
