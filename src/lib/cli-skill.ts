...
export function buildAgentpmSkill(manifest: Manifest): string {
  const lines = [
    ...
    '<!--note:2026-09-15T14:30 高橋-->書いた人の名前も出すなら、日時のあとに空白を挟んで続ける（日時なしで名前だけは付けられない）',
    '',
    '- <!--toggle-->前回の経緯',
    '  - 値段の話',
    '  - 納期の話',
    ...
    '**CLI から本文を `- [x]` に書き換えただけではタスクは完了しない**（見た目が変わるだけ）。',
    '書き換えたあとに、次を実行して追いつかせる:',
    '',
    '```',
    ...
    '```',
    '',
    '- 見るのは**いまチェックが付いている行**。すでに完了のものは飛ばすので、何度実行してもよい。',
    '- **外したチェックは見ない**（完了の取り消しはしない）。戻すときは `task update --status todo`。',
    '- 決まっていない決定事項のタスクは、理由を添えて断られる。**勝手に確定させない** — 決めるのは人。',
    '  確定してよいと言われたら `spec decide` を先に実行する。',
    '- 1件だけ完了にするなら `agentpm task update --task-id <id> --status done` でもよい。',
