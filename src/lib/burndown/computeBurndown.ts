...
export function buildStateAtDate(
  ...
  for (const task of allTasks) {
    stateMap.set(task.id, {
      inMilestone: task.milestone_id === milestoneId,
      status: task.status,
    })
  }

  // If no logs before targetDate, return current state as fallback
  const preLogs = allAuditLogs.filter(
    (e) => toJSTDateString(e.occurred_at) < targetDate
  )
  if (preLogs.length === 0) return stateMap

  // Reset all states — we'll rebuild from event timeline
  for (const [taskId] of stateMap) {
...
async function getHistoricalMilestoneTaskIds(
  ...
  // Find task IDs that were previously assigned to this milestone
  // by searching audit_logs for task.updated events with this milestone_id
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data, error } = await (supabase as any)
    .from('audit_logs')
    .select('target_id')
...
export async function computeBurndown(
  ...
  let endDate = milestone.due_date
  ...
  if (!startDate) {
    // Fallback: use milestone created_at date
  ...
  // 2. Collect all relevant task IDs (current + historical)
  ...
  // 4. Get ALL audit logs (full history for buildStateAtDate)
  const allAuditLogs = await getAuditLogs(supabase, spaceId, allTaskIds)
  ...
  // 6. Filter period logs (>= startDate)
  const periodLogs = allAuditLogs.filter(
    (e) => toJSTDateString(e.occurred_at) >= startDate!
  )
  ...
  while (currentDate <= effectiveEnd) {
    const dayEvents = periodLogs.filter(
      (e) => toJSTDateString(e.occurred_at) === currentDate
    )
