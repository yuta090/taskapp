...
export const INTEGRATION_SETUP_GUIDES: Partial<Record<SetupGuideKey, IntegrationSetupGuide>> = {
  ...
  backlog: {
    ...
    notes: [
      ...
      '古いスペースで使われている「.backlogtool.com」のURLも、TaskApp 側は受け付けます（接続を許可しているドメインに含めてあります）。',
  ...
  asana: {
    ...
    notes: [
      ...
      //    直るまでは正直に書く。
      '⚠ いまの TaskApp では、Asana はまだつながりません。どのワークスペースを見るかを指定する欄が接続フォームに無く、接続はできても同期が始まらないためです。修正中なので、それまでお待ちください。',
    ],
    adminOnly: true,
    sources: [
  ...
  trello: {
    ...
    notes: [
      '利用者がやるのは「リンクを押す → Allow → 出た文字列を貼る」だけです。Power-Up を作ったり API キーを発行したりする作業は不要です。',
      '「Allow」を押しても TaskApp の画面には自動では戻りません。TaskApp は戻り先を指定しない作りにしているため、トークンがその場に表示されます。自分でコピーして貼ってください。',
  ...
  notion: {
    ...
    notes: [
      ...
      '許可したページを使えるのは、許可した本人だけです。Notion の決まりで、同じワークスペースの別の人が使いたいときは、その人も自分で最初から許可し直す必要があります。TaskApp 側は組織にひとつの接続として保存するので、「最初に許可した人が共有したデータベースだけが使える」状態になります。',
