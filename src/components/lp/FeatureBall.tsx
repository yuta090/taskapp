...
export function FeatureBall() {
  return (
    <section id="for-everyone" className="pt-32 pb-20 bg-slate-900 text-white relative overflow-hidden">
      <SkewSeparator position="top" color="fill-slate-50" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center max-w-4xl mx-auto mb-16">
          <div className="text-amber-500 font-bold tracking-wider uppercase mb-4 text-sm">For Everyone</div>
          <h2 className="text-3xl lg:text-4xl font-bold mb-6 leading-[1.15]">
            「誰待ち？」を<br />
            聞く必要がなくなる。
          </h2>
          <p className="text-base text-slate-400 font-medium leading-relaxed">
            すべてのタスクに「ボール」があります。<br className="hidden md:block" />
            確認待ちならクライアントに。作業中なら開発者に。<br className="hidden md:block" />
        ...
        <div className="max-w-3xl mx-auto grid md:grid-cols-3 gap-4 mb-12">
          {[
            { text: 'タスクごとに「client」「internal」のボールラベル' },
            { text: '承認・完了でボールが自動移動' },
            { text: 'MCP経由でもボール操作可能（ball_pass ツール）' },
          ].map((item, i) => (
    ...
    <div className="relative">
      <div className="absolute -top-8 -left-8 -right-8 h-1.5 bg-slate-700 lg:-top-12 lg:-left-12 lg:-right-12">
        <motion.div
          ...
          initial={{ width: "50%" }}
          animate={{ width: ballOwner === 'developer' ? "50%" : "0%" }}
      ...
      <div className="grid md:grid-cols-3 gap-4 items-center relative mt-8">
        ...
        <div className="flex flex-col items-center justify-center gap-6 relative h-full">
          ...
          >
            <div className={`absolute inset-0 rounded-full blur-xl transition-colors duration-500 ${ballOwner === 'developer' ? 'bg-amber-500/30' : 'bg-green-500/30'}`} />
