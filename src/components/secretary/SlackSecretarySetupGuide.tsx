...
export function SlackSecretarySetupGuide({ orgId }: { orgId: string }) {
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      body: (
        <p>
          この下の「合言葉の発行」で相手先（プロジェクト）を選んで発行し、出てきた合言葉を招待したチャンネルにそのまま投稿します。投稿すると「チャンネルの承認」に出ます（30分で失効・1チャンネルに1つ）。
        </p>
      ),
    },
    {
      ...
      body: (
        <p>
          投稿したチャンネルが、この画面の一番下の「チャンネルの承認」に出ます（左メニューの「秘書からの確認」は秘書が拾ったタスク候補用で、こちらではありません）。承認すると、そのチャンネルの会話を秘書が読み始めます。
        </p>
      ),
    },
  ]
