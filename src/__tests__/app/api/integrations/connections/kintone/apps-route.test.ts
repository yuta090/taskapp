...
describe('POST /api/integrations/connections/kintone/apps（アプリの追加）', () => {
  ...
  /**
   * ⚠ 是正した非対称(実装ランナーへの委任事項): 接続作成時(validateKintoneAppCredentials)は
   * トークン1件あたりの長さ上限(200文字)・制御文字を検証していたが、アプリ追加のこの経路には
   * 上限が無く、ボディサイズ上限(8KB)で間接的に縛られているだけだった。同じ検証関数
   * (validateKintoneApiToken)を再利用して揃える。疎通確認(fetchAppFields)より前に弾く。
   */
  it('400: api_tokenが長さ上限(200文字)を超えていれば疎通確認の前に拒否する', async () => {
    const res = await callPost({
      org_id: ORG_ID,
      connection_id: CONNECTION_ID,
      app_id: '9',
      api_token: 'a'.repeat(201),
    })
  ...
  it('400: api_tokenに制御文字が含まれていれば疎通確認の前に拒否する', async () => {
    const res = await callPost({
      org_id: ORG_ID,
      connection_id: CONNECTION_ID,
      app_id: '9',
    ...
    const data = await res.json()
    expect(res.status).toBe(400)
    expect(data.error).toContain('制御文字')
    expect(fetchAppFieldsMock).not.toHaveBeenCalled()
  })

  it('200: ちょうど上限の200文字は許可する', async () => {
    const res = await callPost({
      org_id: ORG_ID,
      connection_id: CONNECTION_ID,
      app_id: '9',
      api_token: 'a'.repeat(200),
    })
  ...
  /**
   * ⚠ IDORテストが空振りしないための直接検証(認可境界。Codexレビュー指摘・Notion側の是正
   * (mapping-save-route.test.ts / mapping-propose-route.test.ts)と同じ形に揃える): 以前は
   * `.eq()` の**キー名だけ**を見ており(`keys.toContain('id')`等)、値(CONNECTION_ID/ORG_ID)を
   * 検証していなかった。これだと実装から `.eq('id', connectionId)` の絞り込みを消して
   * 定数や別の値に変えても、キー名さえ揃っていればテストが通ってしまう(空振り)。
   * 引数を丸ごとcaptureして直接assertする。
   */
