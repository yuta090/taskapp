...
function rawReq(body: string, headers: Record<string, string> = { 'Content-Type': 'application/json' }): NextRequest {
  return new NextRequest('http://localhost/api/integrations/connections/task-sync', {
    method: 'POST',
    headers,
    body,
...
describe('kintone: アプリID(kintone_app_ids)が1件も無い接続は作らせない(死んだ接続防止)', () => {
  ...
  it('kintone_app_idsが1件以上あり、APIトークンの数も一致していれば作成できる', async () => {
    ...
    const res = await POST(
      req({
        org_id: ORG_ID,
        provider: 'kintone',
        // アプリID(kintone_app_ids)の数とAPIトークン(カンマ区切り)の数を一致させる
        // （validateKintoneAppCredentialsが両者の対応づけを要求するようになったため）。
    ...
    expect(res.status).toBe(201)
    // kintoneは一般経路(adapter.listContainers)を使わず、(appId,token)ペアを個別に検証する
    // （下の「アプリとトークンの対応(ペア)検証」ブロック参照）。
    expect(a.listContainers).not.toHaveBeenCalled()
    expect(fetchAppFieldsMock).toHaveBeenCalledWith('https://e.cybozu.com', 'token-5', '5')
    expect(fetchAppFieldsMock).toHaveBeenCalledWith('https://e.cybozu.com', 'token-9', '9')
  })
})

/**
 * kintone: アプリIDとAPIトークンの「対応(ペア)」そのものの検証（外部レビュー指摘・最重要:
 * 「アプリとトークンの対応を検証していない」への是正）。
 *
 * ⚠ 経緯: kintoneのAPIトークンはヘッダにカンマ結合して複数渡せ、kintone側がそのアプリのものを
 * 自動選択する。このため validateKintoneAppCredentials（件数の一致）と旧・adapter.listContainers
 * （カンマ結合トークンでの疎通確認）だけでは、**順序を間違えて貼っても検証が通ってしまう**
 * （kintone側が自動選択するため気づけない）。後で「アプリBを削除」すると、別のアプリ
 * （順序がずれた先）のトークンが消える「死んだ接続」の一種になる。
 *
 * 以後は各(appId, token)ペアを1組ずつ、**そのトークン単体**で fetchAppFields を叩いて検証する。
 */
describe('kintone: アプリとトークンの対応(ペア)そのものを検証する', () => {
  beforeEach(() => {
  ...
  it('順序を入れ替えて貼ったペアは400で拒否される(位置対応の誤りをfetchAppFieldsが検出する)', async () => {
    // appId='5'にはtoken-9(本来9用)が、appId='9'にはtoken-5(本来5用)が渡る想定。
    // fetchAppFieldsは「そのアプリに対してそのトークンが有効か」を1組ずつ検証するので、
    // 順序を間違えたペアは(このテストでは)アプリ5に対して拒否させて再現する。
    fetchAppFieldsMock.mockImplementation(async (_baseUrl: string, token: string, appId: string) => {
      if (appId === '5' && token === 'token-9') {
        throw Object.assign(
          new Error('kintone: このAPIトークンは指定されたアプリのものではありません'),
          { status: 403, permanent: true },
        )
      }
      return [{ code: 'title', label: '件名', type: 'SINGLE_LINE_TEXT' }]
    })
    ...
    const res = await POST(
      req({
        org_id: ORG_ID,
        provider: 'kintone',
        // 本来は 'token-5,token-9' だが、順序を入れ替えて貼った想定。
        api_key: 'token-9,token-5',
        base_url: 'https://e.cybozu.com',
        provider_config: { kintone_app_ids: ['5', '9'] },
      }),
    )
    ...
    // 接続自体を作らない(insertは呼ばれない)。
    expect(insertCapture.provider).toBeUndefined()
  })

  it('1組でも無効なら接続が作られない（INSERTが呼ばれない）', async () => {
    fetchAppFieldsMock
      .mockResolvedValueOnce([{ code: 'title', label: '件名', type: 'SINGLE_LINE_TEXT' }]) // appId=5は有効
      .mockRejectedValueOnce(Object.assign(new Error('kintone: 権限がありません'), { status: 403, permanent: true })) // appId=9は無効
    ...
    expect(res.status).toBe(400)
    expect(insertCapture.provider).toBeUndefined()
    // 2組目(appId=9)で失敗した時点で打ち切る(3組目以降は無いのでこれ以上呼ばれない)。
    expect(fetchAppFieldsMock).toHaveBeenCalledTimes(2)
  })

  it('全ペアが有効なら従来どおり作成できる', async () => {
    fetchAppFieldsMock.mockResolvedValue([{ code: 'title', label: '件名', type: 'SINGLE_LINE_TEXT' }])
    const res = await POST(
      req({
        org_id: ORG_ID,
        provider: 'kintone',
 ...
 * がその契約を守る)。
 *
 * ⚠ 是正済み(Codexレビュー指摘・Critical「正本を欠いた接続を成功扱いにできる」): 以前は
 * トークン数とアプリ数が一致しない入力でも**接続自体は201で成功させ**、kintone_app_tokens
 * (正本)の書き込みだけを諦めていた。これは kintone_app_ids と access_token_encrypted(カンマ結合)
 * はあるのに正本が無い「死んだ接続」そのものであり、以後のアプリ追加/削除がKTGAPで恒久停止し、
 * どのトークンがどのアプリのものか復元不能になっていた。以後は不一致を接続作成そのものの
 * 拒否理由にする(下の「不一致は400で拒否する」テストが、この誤った挙動を固定していた旧テストの
 * 意味を反転させたもの)。
 */
describe('kintone: kintone_app_tokens(トークンとアプリの対応)の作成時書き込み', () => {
  ...
  it('トークン数とアプリ数が一致しない場合は、正本を欠いた「死んだ接続」を作らせず400で拒否する', async () => {
    const res = await POST(
      req({
        org_id: ORG_ID,
        provider: 'kintone',
  ...
  it('保存するkintone_app_idsは検証・正規化後の配列に置き換わる(生のprovider_configをそのまま保存しない)', async () => {
    encryptToken.mockImplementation(async (plaintext: string) => `enc(${plaintext})`)
    const res = await POST(
      req({
        org_id: ORG_ID,
        ...
        api_key: 'token-5,token-9',
        base_url: 'https://e.cybozu.com',
        // number型混在の入力でも、検証・正規化後の文字列配列が保存されること。
        provider_config: { kintone_app_ids: [5, '9'] },
      }),
    )
    expect(res.status).toBe(201)
    expect(insertCapture.import_config).toMatchObject({ kintone_app_ids: ['5', '9'] })
  })

  it('アプリIDが10個(上限9個超)なら接続作成自体を400で拒否する', async () => {
    ...
    const res = await POST(
      req({
        ...
        api_key: tokens,
        base_url: 'https://e.cybozu.com',
        provider_config: { kintone_app_ids: appIds },
      }),
    )
    expect(res.status).toBe(400)
    expect(insertCapture.provider).toBeUndefined()
  ...
  it('重複したアプリIDは400で拒否する', async () => {
    const res = await POST(
      req({
        org_id: ORG_ID,
        provider: 'kintone',
        api_key: 'token-5,token-5b',
        base_url: 'https://e.cybozu.com',
        provider_config: { kintone_app_ids: ['5', '5'] },
      }),
    )
    expect(res.status).toBe(400)
    expect(insertCapture.provider).toBeUndefined()
...
describe('ボディサイズの上限', () => {
  it('Content-Lengthが上限超過なら読む前に413', async () => {
    const response = await POST(
      rawReq('{}', { 'Content-Type': 'application/json', 'content-length': String(16 * 1024 + 1) }),
    )
    expect(response.status).toBe(413)
    expect(insertCapture.provider).toBeUndefined()
  })

  it('実際のボディが上限超過なら413(Content-Lengthを偽っていても本文サイズで弾く)', async () => {
    const hugeProviderConfig = { backlog_note: 'x'.repeat(20 * 1024) }
    const body = JSON.stringify({
      org_id: ORG_ID,
      provider: 'backlog',
      api_key: 'k',
      base_url: 'https://e.backlog.jp',
      provider_config: hugeProviderConfig,
    })
