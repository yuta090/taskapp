 ...
 * 突き合わせるテスト。
 *
 * ActiveOrgProvider の activeOrgId はブラウザの cookie から同期的に読むため、
 * サーバーでは必ず null（document が無い）だが、cookie が既にあるブラウザでは
 * ハイドレーション時の最初の描画から実際の組織IDが入る。このページは
 * useCurrentOrg().loading / orgName / role をそのまま描画に使っているため、
...
describe('OrganizationSettingsPage — サーバーの描画とハイドレーション時の描画の食い違いが無い（React #418）', () => {
  it('cookie由来のactiveOrgId/orgName/roleがハイドレーション直後から確定していても、食い違い警告は出ない', async () => {
