...
// Wiki が空のときの自動作成（ホームページ＋仕様書テンプレート）は、単に空のWikiタブを
// 開いた（読んだ）だけの閲覧者(viewer)にまで書き込みをさせてしまう副作用だった。
// canEdit=false のときは自動作成そのものを行わない。
...
describe('useWikiPages — 空のWikiの自動作成', () => {
  ...
  it('canEdit を渡さない（既定）ときは、これまでどおり空のWikiにホームページ等を自動作成する', async () => {
    const { result } = renderHook(
