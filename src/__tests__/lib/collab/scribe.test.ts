...
describe('書記の決め方', () => {
  ...
  it('1人だけならその人', () => {
    expect(electScribe([{ id: 'tanaka', userId: 'tanaka', joinedAt: 100, collab: true }])).toBe('tanaka')
  })

  it('いちばん古くから居る人になる', () => {
    const peers = [
      { id: 'suzuki', userId: 'suzuki', joinedAt: 300, collab: true },
      { id: 'tanaka', userId: 'tanaka', joinedAt: 100, collab: true },
      { id: 'yamada', userId: 'yamada', joinedAt: 200, collab: true },
    ]
  ...
  it('入った時刻が同じなら、全員が同じ答えになるよう名前順で決める', () => {
    const peers = [
      { id: 'yamada', userId: 'yamada', joinedAt: 100, collab: true },
      { id: 'tanaka', userId: 'tanaka', joinedAt: 100, collab: true },
    ]
    expect(electScribe(peers)).toBe('tanaka')
    // 並び順を変えても同じ答えになる
  ...
  it('在席の一覧だけで決まる（誰が今の書記かを覚えていなくても同じ答えになる）', () => {
    ...
    const peers = [
      { id: 'tanaka', userId: 'tanaka', joinedAt: 300, collab: true },
      { id: 'yamada', userId: 'yamada', joinedAt: 100, collab: true },
    ]
  ...
  it('1人で書く形へ落ちた人は、書記に選ばない', () => {
    ...
    const peers = [
      { id: 'tanaka', userId: 'tanaka', joinedAt: 100, collab: false },
      { id: 'yamada', userId: 'yamada', joinedAt: 200, collab: true },
    ]
    expect(electScribe(peers)).toBe('yamada')
  })
  ...
  it('手前に出ているタブを先に選ぶ（裏のタブは保存が何分も遅れる）', () => {
    ...
    // 新しくても手前のタブに保存を任せる
    const peers = [
      { id: 'tab-old', userId: 'tab-old', joinedAt: 100, collab: true, visible: false },
      { id: 'tab-new', userId: 'tab-new', joinedAt: 200, collab: true, visible: true },
    ]
  ...
  it('どちらも手前なら、これまでどおり古いほうを選ぶ', () => {
    const peers = [
      { id: 'tab-old', userId: 'tab-old', joinedAt: 100, collab: true, visible: true },
      { id: 'tab-new', userId: 'tab-new', joinedAt: 200, collab: true, visible: true },
    ]
  ...
  it('印が無い相手は手前に出ている扱い（今までと同じ順番になる）', () => {
    const peers = [
      { id: 'tab-a', userId: 'tab-a', joinedAt: 100, collab: true },
      { id: 'tab-b', userId: 'tab-b', joinedAt: 200, collab: true, visible: true },
    ]
    expect(electScribe(peers)).toBe('tab-a')
  })

  it('全員が落ちていれば決まらない', () => {
    expect(electScribe([{ id: 'tanaka', userId: 'tanaka', joinedAt: 100, collab: false }])).toBeNull()
  })
})
...
describe('目録に返事をする人', () => {
  const room = [
    ...
    { id: 'c', userId: 'c', joinedAt: 300, collab: true },
  ]
  ...
  it('ほかに誰も居なければ決まらない', () => {
    expect(electAnswerer([{ id: 'a', userId: 'a', joinedAt: 100, collab: true }], 'a')).toBeNull()
  })

  it('落ちた人は返事の係にしない', () => {
    expect(electAnswerer([{ id: 'a', userId: 'a', joinedAt: 100, collab: false }, ...room.slice(1)], 'c')).toBe('b')
  })
})
