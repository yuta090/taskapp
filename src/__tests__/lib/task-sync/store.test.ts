...
describe('createTaskSyncStore.createLinkedTask — 外部由来タスクの作り方', () => {
  it('本文が無いとき description に null を入れない（NOT NULL違反で取り込みが恒久停止するため）', async () => {
    ...
    const store = createTaskSyncStore({ admin, orgId: ORG_ID })
  ...
  it('取り込んだタスクは必ず取り込み元を期限の正本にする（無防備な内部期限に化けさせない）', async () => {
    // 正本が null のタスクは「TaskApp内部で管理している期限」とみなされ、鮮度チェックが一切
    // かからないまま催促が飛ぶ。鮮度SLAを持たないツールは、正本を立てたうえでリマインド側が抑止する。
    const { admin, captured } = stubAdmin()
    await createTaskSyncStore({ admin, orgId: ORG_ID }).createLinkedTask({
      connectionId: CONNECTION_ID,
      task: task(),
      targets: { targetSpaceId: 'space-1' },
      assigneeId: null,
    })
    expect((captured['tasks.insert'] as Record<string, unknown>).due_authority_connection_id).toBe(
      CONNECTION_ID,
    )
  })
...
describe('createTaskSyncStore.markLinkOrphaned — 削除時に期限の正本を外す', () => {
  ...
  it('対応を orphaned にし、あわせて期限そのものを外す（消えたタスクで催促しない）', async () => {
    const { admin, calls } = orphanStub()
    const store = createTaskSyncStore({ admin, orgId: ORG_ID })
    await store.markLinkOrphaned(CONNECTION_ID, 'x1')
    expect(calls.linkUpdate?.state).toBe('orphaned')
    // 正本フラグだけ外すのは逆効果（鮮度チェックの無い内部期限に化ける）。期限そのものを外す。
    expect(calls.taskUpdate).toEqual({ due_date: null, due_authority_connection_id: null })
  })
})
...
describe('createTaskSyncStore.updateLinkedTask', () => {
  it('本文が無いときも description は空文字（null を書かない）', async () => {
    const { admin, captured } = stubAdmin()
    await createTaskSyncStore({ admin, orgId: ORG_ID }).updateLinkedTask(
      'task-1',
      task({ body: null }),
    )
