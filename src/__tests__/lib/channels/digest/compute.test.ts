...
describe('buildApprovalPromptFlexMessage（責任者確認 Stage 2.7-B）', () => {
  const TODAY = '2026-07-14'
  const TASK_ID = '11111111-1111-4111-8111-111111111111'

  it('タイトルと承認/却下ボタン（postback）を含む', () => {
    ...
    const serialized = JSON.stringify(flex)
    expect(serialized).toContain('酒屋へ発注')
    // 承認/却下の postback data（webhook のパーサが解釈できる形式）
    expect(serialized).toContain(`action=digest_promote&task=${TASK_ID}`)
    expect(serialized).toContain(`action=digest_reject&task=${TASK_ID}`)
    // done/undo と取り違えない
    expect(serialized).not.toContain('action=digest_done')
    expect(serialized).not.toContain('action=digest_undo')
  })
  ...
  it('期限があれば添え書きに出る', () => {
    ...
    expect(JSON.stringify(flex)).toContain('7/17')
  })

  it('titleの制御文字は除去してから埋め込む（非信頼入力）', () => {
    const flex = buildApprovalPromptFlexMessage({
      taskId: TASK_ID,
      title: '酒屋へ\n発注',
      dueDate: null,
    ...
    expect(JSON.stringify(flex)).toContain('酒屋へ発注')
  })

  it('altTextにもタイトルを含む（通知プレビュー用）', () => {
    const flex = buildApprovalPromptFlexMessage({
      taskId: TASK_ID,
      title: '発注',
      dueDate: null,
