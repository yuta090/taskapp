...
describe('parseDigestPromotePostback / parseDigestRejectPostback（責任者確認）', () => {
  it('promote 正しい形式を解析する（self なしは assignSelf=false）', () => {
    expect(parseDigestPromotePostback(`action=digest_promote&task=${TASK_ID}`)).toEqual({
      taskId: TASK_ID,
      assignSelf: false,
    })
  })

  it('promote self=1 は assignSelf=true（承認して自分がやる）', () => {
    expect(parseDigestPromotePostback(`action=digest_promote&task=${TASK_ID}&self=1`)).toEqual({
      taskId: TASK_ID,
      assignSelf: true,
    })
    expect(buildDigestPromotePostbackData(TASK_ID, true)).toBe(`action=digest_promote&task=${TASK_ID}&self=1`)
  })
  ...
  it('builder で往復できる', () => {
    expect(parseDigestPromotePostback(buildDigestPromotePostbackData(TASK_ID))).toEqual({
      taskId: TASK_ID,
      assignSelf: false,
    })
