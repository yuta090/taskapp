...
import { useOrgReminderSettings } from '@/lib/hooks/useOrgReminderSettings'
...
export default function OrganizationSettingsPage() {
  ...
  // react-queryの永続キャッシュに乗せることで、開くたびの取り直し・前回値を出せない問題を
  // 解消する（詳細はuseOrgReminderSettings参照）。個人単位の受信オフ
  // (profiles.due_reminder_enabled)は別軸（settings/account側）でそのまま残す。
  const {
    dueRemindersEnabled,
    dueRemindersLoading,
    dueRemindersFetchError,
    dueRemindersSaving,
    dueRemindersSaveError,
    setDueRemindersEnabled,
  } = useOrgReminderSettings(orgId)
  ...
  const handleToggleDueReminders = () => {
    if (!orgId || !isOwner || dueRemindersFetchError) return
    setDueRemindersEnabled(!dueRemindersEnabled)
  }
          ...
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex items-center cursor-pointer flex-shrink-0">
              <input
                ...
                onChange={handleToggleDueReminders}
