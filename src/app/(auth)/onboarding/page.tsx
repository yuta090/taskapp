...
export default function OnboardingPage() {
  ...
  async function handleCreateOrg(e: React.FormEvent) {
    ...
    try {
      ...
      // 流入経路（どこから来て登録したか）を運営の分析用に記録する。first-touch cookie を読んで RPC 1回。
      // 投げっぱなしだと直後の画面遷移で取りこぼすので待つ。失敗は中で握るのでオンボーディングは止まらない
      await recordOrgAcquisition(supabase as SupabaseClient, rpcResult.org_id as string)
