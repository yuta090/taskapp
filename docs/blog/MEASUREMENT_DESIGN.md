...
## 2026-09-07 追記: 組織単位の流入経路（org_acquisition）

記事計測（下記）はそのまま。これとは別に、**組織作成時に流入経路を1行記録する仕組み**を足した
（`org_acquisition`・広告/検索/SNS/紹介/営業なども含む全経路）。運営画面 `/admin/analytics` で
経路別の定着・有料化率が見られる。設計: `docs/spec/ADMIN_FUNNEL_ANALYTICS.md`。
記事別の登録数は引き続き下記の SQL（user metadata）で取れる。
...
- **集計値は厳密な数値ではない**: metadataはクライアント申告のため技術的には偽装可能
  (APIを直接叩けば水増しできる)。マーケ判断用の目安として扱い、課金や監査には使わない
- **CTAの内部リンクは必ず `/` 始まりで登録する**(admin画面のCTA編集時の運用ルール)。
  `https://agentpm.app/...` と絶対URLで書くと外部リンク扱いになり計測から漏れる
- first-touch(localStorage)は無期限。かなり前に読んだ記事が登録に紐づくことがあるが、
  「最初の接点」を知る目的なので仕様として許容
- 計測は自社ドメイン内・第三者提供なしの範囲(cookie同意バナー不要の設計)。
  EU圏ユーザーを本格対象にする場合は同意設計を再検討
