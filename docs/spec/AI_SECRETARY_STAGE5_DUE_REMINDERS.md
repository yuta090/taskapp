...
**確認の問いは「完了しましたか？」1本に統一**（PR-2実装者は問いを増やさない）。ball=client の「催促」は**文面層**で表現し（§9のkind×ball文面）、postbackセマンティクス（[完了した][まだ][再通知]）は kind/ball 非依存で同一。「催促しましたか？」の独立ボタン・状態化は **v2**（催促済みフラグとその検証手段が要るため v1 では持たない）。
...
- **v1(PR-1) は設定UIなし**: グローバル定数 `[-1440, 0, +1440]`＋`SEND_HOUR_JST` で planner が materialize（entitlement-blind）。**設定UI（org/project既定＋タスク単位上書き）は PR-2**。
- **粒度（PR-2）**: 組織/プロジェクトの**既定**（例: 期限1日前＋当日＋超過時）＋ **タスク単位の上書き**（TaskInspector）。設定は TaskApp データ（外部と無関係）。offset_minutes 群として occurrence planner が参照。冗長と感じるユーザーはここでオフ/縮小（§D の冗長性対応レバー）。
...
- **宛先（§A裁定）**: **内側担当者＝`tasks.assignee_id`（催促/対応する側）**。3段固定優先: **(1)** Pro＋`line_direct_dm`＋active user link（`channel_user_links`）→ その口座で **1:1 DM** ／ **(2)** Pro で DM不能 → **発生元チャットグループ**（`chatReplySender`系の解決）優先・無ければ space の active グループ（`selectPreferredActiveGroup`）。文面に担当者名を実名テキストで明記（技術的mentionは使わない）／ **(3)** ルート皆無 → **`suppressed('no_route')`** 終端。
  - **`assignee_id` が NULL のタスクは v1 では occurrence を生成しない**（曖昧な宛先の fallback を作らない＝担当者を付けさせるのが正）。
  - **ball は宛先を変えない**（ball=client でも internal でも宛先は同じ内側担当者。変わるのは文面のみ＝§B: client=催促ナッジ／internal=対応ナッジ）。送信直前に ball を再読取り（§6 の staleness 再読取りと同一クエリ）して文面を決める。
  - digest 用 `resolveAssignee`（LINE-native担当者用）は**ここでは使わない**。正規の逆引きは `channel_user_links`＝`store.ts` に `findActiveUserLinkForUser(orgId, userId)` を追加（`hasActiveUserLinkForUser` の値返し版）。
- **Free**: push ルートに入らず、**channel-digest の期限セクション**（occurrence非依存＝digest生成時に due window を直接クエリ・§6 external鮮度抑止は同適用・追加billable send なし）。digest 期限セクションは **`timed_line_reminders` 非保持 org のみ**に出す（Pro は push と二重になるため出さない）。
...
- 既定リマインドのオフセット群（PR-1 はグローバル定数 **`[-1440, 0, +1440]`**＝1日前/当日/超過1回）。org/project/task 設定は PR-2（§8）。
