...
- **channel-digest の期限セクション＝安全網（中立文面・per-task判定に刷新）**:
  ...
  - **掲載条件をper-task「DMで届かない場合」に変更**（旧v1は「`timed_line_reminders` 非保持org
    ...
    の判定基準と対称化・是正済み）。
  - **⚠ 既知の穴（本PR対象外・次PRで解消予定）**: 上記の「DMルートがある」判定は
    account/紐付けの**状態**（active/revoked）のみを見ており、LINE側で相手がBotをブロックした
    等の理由で**実際には届かない**ケースまでは検知できない。この場合「DMルートあり」と誤判定して
    digestからも除外され、当該タスクはDM・digestのどちらからも実質的に見えなくなる
    （恒久的な可視性喪失）。次PRでpush失敗（恒久4xx等）の観測結果を用いた
    unreachableマーキングを導入し、マーキング済みaccountはdigestの除外判定から外す方針。
