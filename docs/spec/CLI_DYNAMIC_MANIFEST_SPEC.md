...
- **件数の上限**: サーバー側は**直近 10 件だけ残す**（古いものは削除してよい。一度出した分は各利用者の既読に残るので再表示されない）。CLI 側は受け取り 20 件（`NOTICES_MAX`・別サーバーを向けた場合の最後の砦）、一度に表示するのは新しい方から 5 件（`MAX_SHOWN`）で、残りは「ほか N 件」にまとめて既読にする。
- **見ていないのに既読にしない**: stderr が端末につながっていない実行（cron・`2>/dev/null`）や `--json` では表示せず、既読にもしない。`agentpm update` は人が打つコマンドなので常に出す（起動時の事前表示は `update` では抑止し、二重表示を防ぐ）。
- **同時実行**: 既読の記録は「読む → 出す → 書く」なので、2 プロセス同時だと片方の記録が上書きされ、同じお知らせがもう一度出ることがある（害はそれだけ。原子的書き込み自体は tmp → rename）。
- **届くまでの遅れ**: サーバーの `Cache-Control: max-age=86400` と CLI 側キャッシュ 24 時間が重なり、最長 2 日遅れる。障害告知など急ぎの用途には使わない。
