...
```
- shared_group_claim（web_approval）: 128bit乱数・**TTL 10-30分**・1グループのみ成功・コード投入では消費せずWeb承認トランザクションで消費。
- code_only: hash-at-rest/128bitは同一、**TTLだけ別＝既定7日/最大30日**（店舗が数日かけてbot追加する現実に合わせる）・**単回成功**（consumed_at）・マルチユース禁止。
...
- channel_accounts の owner_type/org_id 整合 CHECK。
...
- 同一グループへの2claim同時承認は `channel_groups_active_unique` が最終審判。敗者の 23505 は graceful reject（リトライしない）。デッドロック無し。
- RPC内で再検証: purpose/binding_mode・未消費未失効(TTL)・claim.account_id が対象 platform account・**承認者の membership**（API routeが auth.uid をサーバ側解決して渡す。クライアント申告の user_id/org_id は信用しない）。
