...
### PR4 実装状況（2026-07-16・レビュー2巡=Opus code-reviewer＋Codex 収束後に確定）
- **実装済み**: `channel_messages.billable_push`（push配信のみtrue・**集計は status='sent' のみ**＝失敗/queuedの過大計上を排除）＋部分index。純SQL cron `app_refresh_channel_metering_state()`（毎時・(org,当月JST)billable数 vs `monthly_push_quota` → state ok/soft(80%)/hard・quota NULL は ok 正規化）。`app_jst_current_month_bounds()`・console読取 `app_org_channel_push_usage_current_month()`（自org内部メンバー限定=42501）。
- **執行**: `decideAutoPush({state,onExceed,jstDayOfYear})` 真理値表（none=常send＝全既定org実質no-op＝退行ゲート／degrade×soft=隔日／degrade・block×hard=suppress）。送信境界=digest cron・approval-notify cron・**mention即時1:1 approval push**（cron版と対称にgate＋計上）。billable insert は決定的 `externalMessageId`（`buildDigestRetryKey`）で `channel_messages_dedupe` により**二重起動でも二重計上しない**。suppress時は `approval_notified_at` を戻し通知ロストを防ぐ。
- **計上する送信境界**: digest / approval-notify（cron）/ mention即時approval / console手動送信。**計上しない（後続許容・per-org帰属不能）**: platform 1:1 follow（org未解決）・limbo group-join（org未解決）・room離脱前guidance。
- **既知フォローアップ（PR4では対応せず・後続）**:
  1. **push成功後の `insertChannelMessage` 失敗による過小計上**（digest経路）。approval経路は un-claim→cron再送で自己修復するが digest は日次で再pickup機構が無い。堅牢化は send outbox 化（push前に `billable_push=false` の行を作り送信確定で `sent` へ遷移）で対応予定。頻度は稀・被害有界（1通/失敗）。
  2. **account軸(200/account)の相乗り監視/アラート**。データ（`billable_push` 行に `account_id`）は取得済み・per-org 執行は完備だが、複数orgが同一共有accountに相乗りして無料枠200を超える状況の**account横断集計・アラートUI は未実装**（本部ops向け・監視面）。執行(per-org quota)とは独立した観測面として後続。
  3. 隔日縮退の年境界2連休（12/31奇→1/1奇）＝軽微・認識のみ。
...
**`status='disabled'` の意味論（Fable裁定・確定）**: disabled は「活性化(Step6)以前への回帰」と等価＝**新規テナント確立(redeem / web_approval pending登録 / 承認RPCによるgroup作成)を凍結**。凍結は「拒否」でなく「不作為」＝コードを**消費しない**（`findValidSharedGroupClaimCode` すら呼ばない）ので、再有効化後に同一コードで正常成立する（transient flap でのコード無駄消費なし）。
- webhook: `processPlatformLimboGroupMessage` 先頭で `if (disabled) return`（redeem/pending登録/limboのevents_seen等メタ書込を凍結・reply も無し）。
- 承認RPC(`rpc_approve_group_claim`)・redeem RPC(`rpc_redeem_code_only_claim`): **対象 account が disabled なら拒否**（コンソール承認が凍結の裏口になるのを塞ぐ・防御的にDB層でも強制）。
- 成立通知メール: 成立自体が起きないため飛ばない。
- **紐付け済み(active世代)group の inbound 記録は従来どおり継続**（Stage1原則＝証跡不可侵。reply/digest/送信のみ停止）。緊急停止＝「新規確立の凍結」と証跡不可侵＝「確立済みの記録継続」は対象集合が排反で両立。
- webhook は disabled 中も常に 200（LINE再送嵐回避）。
