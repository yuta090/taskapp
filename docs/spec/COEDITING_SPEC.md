...
6. **カーソルの色は部屋の中で割り当て、一度決めたら会期中は変えない**（`colorIndexOf`）。
   人ごとにハッシュで選ぶと運が悪いと2人が同じ色になる。並べ方は**入った順 → 名札順**だけで
   決める（書記の決め方とは分ける。あちらは「手前に出ているか」で先頭が入れ替わるので、
   誰かがタブを切り替えるたびに全員の色がずれてしまう）。
   **途中で変えられない理由**: カーソルを描く部品は相手ごとに札を1回だけ作って使い回すので、
   あとから色を変えても相手の画面には届かない（帯だけ新しい色になり、カーソルは古い色のまま）。
   受け入れている残り: 誰かが抜けたあとに別の人が入ると、番号が重なることがある。
   開き直せば直る。
