...
| `src/lib/collab/scribe.ts` | 書記の選び方（いちばん古い人・いまの書記は替えない）と、保存の基準の受け渡し |
...
5. **本文を作るのは「本当に自分ひとり」のときだけ。** 在席には2つの印を載せる —
   `present`（いま開いていて加わるつもり。器の用意より**先に**立つ）と `collab`（本文を
   持っている）。`collab` だけを見ると、ほぼ同時に開いた2人が互いに気づかず、どちらも
   列から本文を作る。合流すると**中身が二重**になり、「片方を消してももう片方が残る」
   「相手の編集が下側の複製に入って見えない」「気づいた側が画面を読み直す」が同時に
   起きる。実際にユーザーから3つとも報告された（2026-09-17）。
...
- 種が2つ入ったときは、帯を出さずに**列から読み直す**（器の本文が二重になっているので、それを保存させない）。
...
- 在席の記録だけ残った幽霊が居ると、新しく入った人が4秒読み取り専用で待つ（返事が来ないため）。
