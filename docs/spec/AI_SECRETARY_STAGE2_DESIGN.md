...
## 10. マルチチャネル展開（Slack / Chatwork / Google Chat）

Stage 1 スキーマは全表が `channel` 列を持ち、ログ・グループ・digest・消し込みはチャネル非依存。**Stage 2 実装の要件として、LINE固有処理を `ChannelAdapter` インターフェースに分離する**:

```
ChannelAdapter（チャネルごとに1実装）
  verifyWebhook(raw, headers)      -- 署名検証
  normalizeEvents(raw)             -- 受信イベント → NormalizedEvent[]
  sendText(target, text)           -- テキスト送信
  sendDigest(target, items)        -- digest描画（ボタン可否はチャネル別）
  fetchAttachment(ref)             -- 添付取得
```

| チャネル | グループ | 消し込みUI | 個人DM | 備考 |
|---------|---------|-----------|--------|------|
| LINE（実装対象） | ◎ | Flexボタン＋「完了N」返信 | 友だち追加が前提（二層設計 §6） | push課金・reply無料 |
| Slack | ◎ チャンネル | Block Kitボタン | ◎ 制約なし | **既存Slack連携（webhook/署名検証/送信/チャンネル⇔space紐付け）を流用可能。追加優先度1** |
| Chatwork | ◎ ルーム | 「完了N」返信＋**Chatwork純正タスクにAPI起票** | ◎ | 実装軽量。士業・中小と相性◎。優先度2 |
| Google Chat | ◎ スペース | カードボタン | ◎ | Workspace前提・アプリ配布に一手間。優先度3 |
| メール | − | 層2アクショントークンのリンク | ◎ | 受信基盤が必要（v0.1計画どおり） |

- LINE以外は「友だち追加」の壁と通数課金が無いため、二層設計（§6）はLINE固有。identity はワークスペースのユーザーIDで最初から記名。
- 白ラベルの形: LINE=事務所ごとの公式アカウント／Slack・Google Chat=当社1アプリを各ワークスペースへインストール／Chatwork=事務所ごとのbotアカウントで白ラベル維持可。
