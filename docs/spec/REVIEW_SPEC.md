...
- プロジェクト設定 → **社内承認** で、そのプロジェクトの「既定の承認者」を選べる。
- 保存先は `spaces.default_reviewer_ids`（uuid[]）。可視性・更新権限は既存の spaces RLS に従う。
- タスク詳細の社内承認で承認者を選ぶとき、この人たちが**最初から選ばれた状態**になる。
  あくまで初期値で、その場で足したり外したりできる（固定ではない）。
- 承認者を選ぶ画面の各行にも「デフォルト承認者」チェックがあり、**選択中の人だけ**に出る。
  ここで付け外ししても同じ設定（プロジェクト設定）が書き変わる。
- 既定の人がスペースを抜けた場合や、依頼者自身が既定に入っている場合は無視する
  （`resolveDefaultReviewerIds` が「いま依頼できる人」に絞る）。
- **再依頼のときは既定を使わず、前回の承認者を引き継ぐ**（差し戻した人にもう一度見てもらうのが自然なため）。

## 依頼するとタスクの状態も「社内承認中」になる（2026-09-14〜）

以前は依頼と状態が別々の操作だった。「社内承認を依頼」を押しても `tasks.status` は元のままで、
逆に status を `in_review` にしても依頼は飛ばない。2回操作しないと揃わず面倒なので、
使われなくなっていた（ユーザー指摘）。

- 依頼が `open` になったら、そのタスクを `status='in_review'` にする。
- 正本は DB のトリガー `trg_sync_task_status_on_review_open`（`reviews` の AFTER INSERT/UPDATE OF status）。
  画面・CLI・今後の経路のすべてで同じ結果になる。
- 既に `in_review` なら触らない（`updated_at` をむだに進めない）。
- **`done` のタスクは戻さない**（完了後に依頼を作り直しても完了を取り消さない）。
- **承認しても完了にはならない**。完了にするのは人。
- **差し戻しでも状態は戻さない**（直す作業が続くため）。ボールだけ社内に戻るのは従来どおり。

逆方向（状態を `in_review` にしたら依頼が飛ぶ）は**やらない**。誰に頼むかが決まらないと
依頼を作れないため。画面は「`in_review` なのに依頼が無い」とき承認者を選ぶ欄を自動で開いて促す。

画面のキャッシュも同じ判断で先出しする（`src/lib/tasks/reviewStatusSync.ts`）。
検証は `supabase/tests/run_review_open_status.sh`（5項目）。
...
### AT-001 会議作成（参加者任意）
- [x] ~~クライアント参加者0名では保存できない~~ → 外部参加者0名でも会議作成可能に変更（社内ミーティング対応）
- [x] 1名以上で保存でき、会議一覧に `planned` で出る
- [x] 外部参加者なしの社内ミーティングも作成・開始・終了が可能
