...
## 会議（議事録ページ）
- 会議中に表示する **議事録ページ** を用意する（クライアントにも同じページを共有できる）。
- 議事録ページは **3カラム**：
  - 議題/メモ（会議中のメモ）
  - 未決定（検討中・未承認の一覧）
  - 決定（会議で決めた内容の一覧）
- 会議開始/終了を明示する（「会議中」バッジ表示）。クライアント側メニューにも会議を追加。
- 会議中の「未決定の解決（決定/承認）」は **開発会社側の責任者が代理入力可能**。
  - ただし **確認者（クライアント）を選択必須**（誰が合意したかのログが壊れるため）
  - 確認者未選択の場合は「仮決定」として未決定に残す（後で確定させる）
- 会議終了時に、決定事項と宿題を **メール＋アプリ内通知**でまとめて送る（議事録通知）。
