...
| **Warning** | orange-600 | `text-orange-600` | `bg-orange-50` | 注意、警告、要対応、接続異常 |
| **Danger** | red-600 | `text-red-600` | `bg-red-50` | エラー、削除、期限切れ、緊急 |
| **Info** | blue-600 | `text-blue-600` | `bg-blue-50` | 情報、進行中ステータス |
| **Client** | **amber-500** | `text-amber-500` | `bg-amber-50` | クライアント可視要素（**この意味専用**。CLAUDE.md と一致） |

> **amber は「クライアント可視」専用**（#88）。注意/警告/要対応は amber ではなく **orange（Warning）**、
> 緊急は **red（Danger）**、保存中など一時状態は **中立(gray)** を使う。承認の主アクションは **green（Success）**。
> セマンティッククラスは `src/lib/design/tokens.ts`（`CLIENT` / `WARNING` / `SAVING` / `APPROVE_BUTTON`）を参照する。
> 注意度のエスカレーション: gray（一時） < **orange（要対応）** < **red（緊急）**。
...
| `amber-*` を警告/要対応/保存中/緊急に流用 | `WARNING`(orange) / `SAVING`(gray) / `red`(緊急) | amberはクライアント可視専用（#88） |
| 承認ボタンに indigo/emerald/amber | `APPROVE_BUTTON`(green) | 面ごとの色違いを禁止（#88） |
