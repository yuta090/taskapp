...
プロジェクトの Wiki に初めてアクセスした際、ページが0件の場合は以下のテンプレートページが自動で生成されます。

#### 1. プロジェクトホーム

プロジェクトの中心となるページです。以下のセクションが含まれます。
...
データベーススキーマの定義テンプレートです。

- テーブル一覧
- テーブル定義（カラム名、型、NOT NULL、説明）
- リレーション
- インデックス
...
- 画面一覧（画面名、パス、説明、ステータス）
- デザインシステム情報
- 画面仕様テンプレート

> **Tips**: テンプレートは初回アクセス時に1度だけ生成されます。ページが既に存在する場合は生成されません。
...
1. Wiki に初めてアクセスする（テンプレートが自動生成される）
