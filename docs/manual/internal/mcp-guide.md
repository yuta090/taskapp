...
MCP（Model Context Protocol）は、AI のチャットから AgentPM を直接操作するための決まりごとです。つなぐと、話しかけるだけでタスクの作成・更新、会議の管理、レビュー操作ができます。

つなぎ方は2つあります。**ふつうは「外部チャットからつなぐ」を使ってください。**

| つなぎ方 | 向いている相手 | 入れるもの |
|---|---|---|
| **外部チャットからつなぐ**（推奨） | ChatGPT・Claude のウェブ版・Cursor・VS Code など | URL を1本貼るだけ |
| 手元のパソコンで動かす | Claude Code（開発者向け） | Node.js と鍵の設定 |

---

## 外部チャットからつなぐ（推奨）

AgentPM のサーバーに接続先が用意してあるので、インストールは要りません。

### 接続先

```
https://agentpm.app/api/mcp
```

### ChatGPT の場合

1. ChatGPT の設定 → コネクタ → 開発者モードを有効にする（Plus・Pro・Business などのプランが必要）
2. 「カスタムコネクタを追加」に上の URL を貼る
3. AgentPM のログイン画面が出るので、ログインする
4. 許可の画面で、**つなぐ組織**と**できることの範囲**を選んで「許可する」

### 許可する範囲

| 選択肢 | できること |
|---|---|
| 見るだけ | タスク・会議・議事録・Wiki を読む |
| 見る＋書く | 上に加えて、タスクの作成・更新・ボール渡し |

**どちらを選んでも、削除・一括変更・承認はできません。** 承認は責任者本人の行為なので、外部のチャットには任せない作りにしています。

### 接続の確認と解除

設定 → 外部チャットとの接続（`/settings/connections`）で、いま何がつながっているかを見られます。心当たりのない接続はここで解除してください。組織のオーナーは、その組織の全員分が見えます。
...
**リモートMCPはフリーでも使えます。** 上限に当たったら、使っていない接続を解除すれば新しくつなげます。すでにつないである接続は、プランが下がっても切れません。

### 注意

- 相手先・協力会社のアカウントからは接続できません（社内メンバー専用）
- 1つの接続で見られるのは、選んだ**1つの組織**だけです。複数の組織を使うなら、つなぎ先でもう1つ接続を作ってください

---

## 手元のパソコンで動かす（Claude Code 向け）
...
| ツール名 | 説明 |
|---------|------|
| `activity_search` | データの変更履歴（誰が・いつ・どの経路で・何を変えたか）を検索 |
| `activity_entity_history` | 1つの行（タスク・Wikiページ等）の変更履歴を取得 |
...
画面・CLI・MCP のどこからの変更も、データベースが自動で変更履歴（`change_log`）に記録します。誰が・いつ・どの経路で・何を変えたかが残るので、`activity_search` / `activity_entity_history` で調べられます。合言葉などの秘密は記録に写しません。
