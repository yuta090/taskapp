...
| **元栓**（`STRIPE_SELF_SERVE_ENABLED`） | 新規のオンライン申し込みを受け付けるか | 「Proにアップグレード」ボタンと `/api/stripe/checkout` が開くか |

**両方そろって初めて新規の申し込みが開く。**元栓は既定で閉じているので、鍵を入れただけでは開かない。
...
1. 「設定 → プランと請求」で **「Proにアップグレード」が押せる**（元栓を開けた Preview で）
