...
-- 待つ上限: realtime.messages は Realtime が日ごとの子表を作るなどで押さえることがある。長く待ち続けて Realtime の
--   読み書きを後ろに詰まらせないよう、3 秒で諦めて全体を取り消す（apply-migration.sh の 1 トランザクション内で効く。
--   トランザクションで包まずに流す空 DB の確認では、警告が出るだけで何も変わらない）。
