...
--   しても、相手先・vendor の受信トレイにタスク名とコメントの先頭120文字が残る。コメント自体は RLS で読めなくなるのに、
--   お知らせからは読めてしまう。
...
--   節 4: 末尾の確認（何も変えない）
--
-- 範囲外: ボールの受け渡し（ball だけの変化）では消さない。ユーザー判断で、塞ぐべき漏れとして扱わない（節 3 の理由を参照。
--   トリガーに ball を戻さないこと）。
--   space のメンバーから外した・space や組織の役割を変えたときの判定し直し（ほかのお知らせの種類と共通の話なので、別に扱う）。
