...
begin
  ...
  -- タスクを相手先に公開（公開時刻は残らないため、作成/更新の早い方で近似）
  insert into org_milestones (org_id, milestone, reached_at)
  select t.org_id, 'task_published', min(coalesce(t.updated_at, t.created_at))
  from tasks t
  ...
  -- AI連携を設定
  insert into org_milestones (org_id, milestone, reached_at)
  select c.org_id, 'ai_configured', coalesce(c.created_at, c.updated_at, now())
  ...
  -- 7日後 / 30日後も利用（組織作成から N 日以上たった後の、サンプル以外のタスク作成/更新）
  insert into org_milestones (org_id, milestone, reached_at)
  select a.org_id, 'retained_7d', min(a.ts)
  from (
    select t.org_id, t.created_at as ts from tasks t where t.is_sample = false
    union all
    select t.org_id, t.updated_at from tasks t where t.is_sample = false and t.updated_at is not null
  ) a
...
begin
  ...
  v_ref := nullif(left(coalesce(p_data->>'ref', ''), 40), '');
  v_art := nullif(left(coalesce(p_data->>'article_slug', ''), 64), '');
  ...
  if v_channel is null or v_channel = 'direct' then
    ...
    if v_meta ? 'signup_ref' then
      v_ref := coalesce(v_ref, left(v_meta->>'signup_ref', 40));
      v_art := coalesce(v_art, left(v_meta->>'signup_art', 64));
  ...
  if v_channel is null or v_channel not in (
    ...
    'email','sales','event','direct','other','unknown'
  ) then
    v_channel := 'direct';
  end if;
  ...
  insert into org_acquisition (
    ...
    case when (p_data->>'first_touch_at') ~ '^\d{4}-\d{2}-\d{2}T' then (p_data->>'first_touch_at')::timestamptz else null end,
    v_uid
  )
...
-- -----------------------------------------------------------------------------
create or replace function public.admin_org_milestone_stats(p_since timestamptz default null)
returns table (
  channel text,
  milestone text,
  org_count bigint,
...
revoke execute on function public.admin_org_milestone_stats(timestamptz) from public, anon, authenticated;
grant execute on function public.admin_org_milestone_stats(timestamptz) to service_role;
...
--   drop function if exists public.admin_org_milestone_stats(timestamptz);
