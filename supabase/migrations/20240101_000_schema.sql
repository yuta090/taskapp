...
-- =============================================================================
-- GitHub 連携の表（github_installations / github_repositories / space_github_repos /
-- github_webhook_events / github_pull_requests / task_github_links）は
-- 20240205_000_github_integration.sql で定義する。
-- 以前はここに同じ名前で列の違う下書きの定義があり、空のDBから流すと
-- 20240205 の create table if not exists が飛ばされて、github_repo_id の索引で止まっていた。
-- 本番の表は 20240205 の形（2026-09-10 に列を照合済み）なので、ここから外しても本番は変わらない。
