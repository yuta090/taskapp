...
-- （dedupe_key が札と人で決まるので、会議終了で届いたものは期限超過で重ねて届かない）。
--
...
-- 誰に届けるか
-- ---------------------------------------------------------------------------
-- その札の**ボールを持っている側**の担当者（task_owners.side = tasks.ball）。
-- 相手先が決めることなら相手先に、社内で決めることなら社内の担当に届く。
-- 担当が1人もいない札は誰にも届かない（宛先が決められないため。作成者へは送らない
...
BEGIN
  ...
  inserted AS (
    ...
    SELECT
      ...
      -- 札と人で1回だけ。会議終了で届いたものは、期限超過では重ねて届かない
...
COMMENT ON FUNCTION public.process_spec_decision_nudges() IS
  '未決の決める札について「決めてください」を受信トレイに入れる（期限超過・会議終了の2つのきっかけ。札と人で1回だけ）';
...
--   2. 期限切れの未決の決める札を1つ作る → 15分以内に、ボールを持つ側の担当の受信トレイに出る。
--      開くと「決定にする」が押せる。
--   3. もう一度流しても増えない（札と人で1回だけ）。
--   4. 決定済み・完了・担当のいない札には出ない。
--
-- ロールバック:
