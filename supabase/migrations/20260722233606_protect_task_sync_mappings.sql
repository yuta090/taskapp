...
--   ⚠ 補足: auth.role() は Supabase 側で非推奨（deprecated）扱いになっている。ここで使い続けて
--     いるのは、このリポジトリの既存ガード(a)と判定を揃えるため（片方だけ新方式に寄せると
--     系統が3つに増えて、どの経路がどれで通るのかが余計に分かりにくくなる）。将来 (a) 系統を
--     まとめて置き換えるときに、ここも同時に移すこと。(b) だけでも service_role の判定自体は
--     成立するため、auth.role() が将来 NULL しか返さなくなっても fail-closed にはならない。
