...
-- API が台帳外キーを 400 で拒否する。DB は形式（小文字英数字と _ 、64文字以内）だけ守る。
...
declare
  c text;
begin
  -- 旧「キー列挙」の CHECK だけを対象にする（名前には依存しない。他の CHECK は触らない）
  for c in
    select conname from pg_constraint
    where conrelid = 'public.email_templates'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) like '%invite_client%'
  loop
...
alter table public.email_templates
  add constraint email_templates_key_format check (key ~ '^[a-z0-9_]{1,64}$');
