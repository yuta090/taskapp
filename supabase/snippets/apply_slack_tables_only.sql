...
create index if not exists space_slack_channels_space_idx on space_slack_channels(space_id);
...
create unique index if not exists slack_message_logs_dedupe_unique on slack_message_logs(dedupe_key) where dedupe_key is not null;
create index if not exists slack_message_logs_space_idx on slack_message_logs(space_id, created_at desc);
create index if not exists slack_message_logs_task_idx on slack_message_logs(task_id) where task_id is not null;
...
create policy "org members can view slack workspaces" on slack_workspaces for select
...
create policy "org owners can manage slack workspaces" on slack_workspaces for all
  using (org_id in (select om.org_id from org_memberships om where om.user_id = auth.uid() and om.role = 'owner'));
...
create policy "space members can view slack channels" on space_slack_channels for select
...
create policy "space admins can manage slack channels" on space_slack_channels for all
  using (space_id in (select sm.space_id from space_memberships sm where sm.user_id = auth.uid() and sm.role in ('admin', 'editor')));
...
create policy "space members can view message logs" on slack_message_logs for select
  using (space_id in (select sm.space_id from space_memberships sm where sm.user_id = auth.uid()));

-- 5) トリガー
create or replace function update_slack_updated_at() returns trigger as $$
begin new.updated_at = now(); return new; end;
$$ language plpgsql;

drop trigger if exists slack_workspaces_updated_at on slack_workspaces;
create trigger slack_workspaces_updated_at before update on slack_workspaces for each row execute function update_slack_updated_at();
drop trigger if exists space_slack_channels_updated_at on space_slack_channels;
create trigger space_slack_channels_updated_at before update on space_slack_channels for each row execute function update_slack_updated_at();
...
create or replace function encrypt_slack_token(token text, secret text) returns text as $$
  select encode(pgp_sym_encrypt(token, secret), 'base64');
$$ language sql security definer;

create or replace function decrypt_slack_token(encrypted text, secret text) returns text as $$
  select pgp_sym_decrypt(decode(encrypted, 'base64'), secret);
$$ language sql security definer;

-- 7) channel_id インデックス
create index if not exists idx_space_slack_channels_channel_id on space_slack_channels (channel_id);
