#!/usr/bin/env bash
# =============================================================================
# task_pricing の space の外部キーを ON DELETE CASCADE にそろえる（*_task_pricing_space_cascade.sql）の検証ハーネス
#
...
#   checks      task_pricing_cascade_assert.sql（外部キーの形・space とタスクを消したときの行・見積の見張り）
# 続けて RED=1 でないときだけ:
#   scope_*     本 migration で変わるのは task_pricing の2本の外部キーだけ
#   reapply_*   データが入った状態で再適用できる
#   rollback_*  ロールバック節を流すと、本 migration の前のスキーマと挙動に戻る → そのあと再適用できる
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_task_pricing_cascade.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_task_pricing_cascade.sh    # 本 migration を流さずに同じ assert を回し、
...
ASSERT="$TST/task_pricing_cascade_assert.sql"
RED="${RED:-0}"

shopt -s nullglob
targets=("$MIG"/*_task_pricing_space_cascade.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/tpc.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54455
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "TASK PRICING CASCADE CHECKS FAILED"; then
...
echo "== checks: task_pricing_cascade_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: only the two task_pricing space foreign keys change =="
CHANGED="$(diff "$WORK/fp_before.txt" "$WORK/fp_after.txt" | sed -nE 's/^[<>] //p' || true)"
OUTSIDE="$(printf '%s\n' "$CHANGED" | grep -v '^$' | grep -vE '^con task_pricing task_pricing_space_(id|org)_fkey ' || true)"
NCH="$(printf '%s\n' "$CHANGED" | grep -cE '^con task_pricing task_pricing_space_(id|org)_fkey ' || true)"
if [ -z "$OUTSIDE" ] && [ "$NCH" -eq 4 ]; then
  record "PASS[scope_only_task_pricing_space_fkeys_changed]: 2 foreign keys (4 lines), nothing else"
else
  record "FAIL[scope_only_task_pricing_space_fkeys_changed]: $NCH lines; other changes: $(printf '%s' "$OUTSIDE" | head -5 | tr '\n' ';')"
fi
...
# 「-- ロールバック（節 N」の見出しから次の「-- ====」までのうち、行頭が「--   」の行だけを SQL として取り出す
awk -v dir="$RBDIR" '
...
if [ "$NBLK" -eq 1 ] && [ "$NSTMT" -eq 6 ]; then
  record "PASS[rollback_sections_found]: $NBLK section, $NSTMT statements"
else
  record "FAIL[rollback_sections_found]: $NBLK sections, $NSTMT statements (want 1 section, 6 statements)"
fi
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "TASK PRICING CASCADE CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL TASK PRICING CASCADE CHECKS PASSED (on real migrations)"
