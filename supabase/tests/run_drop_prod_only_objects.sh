#!/usr/bin/env bash
# =============================================================================
# 本番にだけある物を無くす（*_drop_prod_only_objects.sql）の検証ハーネス
#
...
# 本 migration の前に drop_prod_only_objects_seed.sql を入れる（空の DB には無い「本番にだけある物」を、
# 2026-09-12 の点検で記録した本番の形で作る＋残る物のデータ）。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      drop_prod_only_objects_assert.sql（表・列・関数・ポリシーが無くなる／マイルストーンの行と
#               画面の操作は今までどおり／二要素のポリシーの漏れなし）
# 続けて GREEN のときだけ:
#   scope_*     無くなるのは user_preferences 一式と milestones.status だけ（増える物は無い）
#   reapply_*   データが入った状態で再適用できる（2回目は何もしない）
#   rollback_*  ロールバック節を流すと、適用前のスキーマと挙動に戻る → 戻したあと再適用できる
#   lock_*      1回目は無くす表を access exclusive で押さえる（表・列を無くすため。落とした表はカタログから
#               消えるので一覧には milestones だけが出る）。2回目は無くす物が無いのでロックを取らない
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_drop_prod_only_objects.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_drop_prod_only_objects.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_drop_prod_only_objects.sh
...
SEED="$TST/drop_prod_only_objects_seed.sql"
ASSERT="$TST/drop_prod_only_objects_assert.sql"
RED="${RED:-0}"
# 増える物は無い。無くなるのは user_preferences 一式（表・列・制約・索引・ポリシー・トリガー・関数）と
# milestones.status
NEW_RE='^NOTHING-IS-ADDED$'
CHANGED_RE='user_preferences|^col milestones\.status '
WANT_ADDED=0
WANT_REMOVED=22
...
shopt -s nullglob
targets=("$MIG"/*_drop_prod_only_objects.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/dpo.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54470
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "DROP PROD ONLY CHECKS FAILED"; then
...
echo "== checks: drop_prod_only_objects_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: only the production-only table and column are removed; nothing is added =="
...
echo "== locks: first apply holds AccessExclusive on milestones and user_preferences; second apply holds none =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  AE="$({ grep 'AccessExclusiveLock' "$WORK/locks_$k.txt" || true; } | cut -d= -f1 | sort | tr '\n' ',' | sed 's/,$//')"
  # 1回目は無くす表だけを access exclusive で押さえる。2回目は無くす物が無いのでロックを取らない
  # 落とした表はカタログから消えるので、ロックの一覧には出てこない（見えるのは milestones だけ）
  if [ "$k" = first ]; then WANT_AE="milestones"; else WANT_AE=""; fi
  if [ "$AE" = "$WANT_AE" ]; then
    record "PASS[lock_${k}_apply_access_exclusive_as_expected]: ${L:-no table locks}"
  else
    record "FAIL[lock_${k}_apply_access_exclusive_as_expected]: AccessExclusiveLock on ${AE:-none}, want ${WANT_AE:-none} ($L)"
  fi
done
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "DROP PROD ONLY CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL DROP PROD ONLY CHECKS PASSED (on real migrations)"
