...
#
# 使い方: bash supabase/tests/run_shared_bot_metering.sh
...
OUT="$WORK/o.out"
PGOPTIONS='--client-min-messages=notice' psql "$CONN" -v ON_ERROR_STOP=1 -f "$TST/shared_bot_metering_data.sql" > "$OUT" 2>&1 \
...
grep -q "METERING CHECKS PASSED" "$OUT" || { echo "NOT PASSED"; tail -30 "$OUT"; exit 1; }
echo ""
echo "ALL METERING CHECKS PASSED (on real migrations)"
