#!/usr/bin/env bash
# =============================================================================
# 「誰がやったか」を受け取る形の RPC（*_mcp_rpc_as.sql）の検証ハーネス
#
...
# 本 migration の前に mcp_rpc_as_seed.sql（視点ごとの人・タスク・レビュー・会議・日程の提案）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      mcp_rpc_as_assert.sql（形・実行権・判定の補助・7本それぞれで、画面の関数と道具用の関数の結果が同じ）
# 続けて GREEN のときだけ:
#   scope_*     増えるのは関数 15（本体7・道具用7・判定の補助1）で、変わるのは画面から呼ぶ7つの本文だけ
#   reapply_*   データが入った状態で再適用できる
#   rollback_*  ロールバック節を流すと、適用前のスキーマと挙動に戻る → 戻したあと再適用できる
#   lock_*      本 migration を流した（初回と2回目の）トランザクションの最後に、同じ接続が public の表に
#               AccessExclusiveLock を持っていない（関数の置き換えだけ）
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_mcp_rpc_as.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_mcp_rpc_as.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_mcp_rpc_as.sh
...
SEED="$TST/mcp_rpc_as_seed.sql"
ASSERT="$TST/mcp_rpc_as_assert.sql"
RED="${RED:-0}"
# 増える物（本体7・道具用7・判定の補助1）と、変わる物（画面から呼ぶ7つの本文）
NEW_RE='^fn (_[a-z_]+_impl|rpc_[a-z_]+_as|_actor_can_write_space|rpc_(pass_ball|meeting_start|meeting_end|review_open|review_approve|review_block|confirm_proposal_slot))\('
CHANGED_RE='^fn rpc_(pass_ball|meeting_start|meeting_end|review_open|review_approve|review_block|confirm_proposal_slot)\('
WANT_ADDED=22
WANT_REMOVED=7
# ロールバック節の数と、; で終わる行の数（節 1〜8。関数の本文の行も数える）
WANT_RB_BLOCKS=8
WANT_RB_STMTS=251

shopt -s nullglob
targets=("$MIG"/*_mcp_rpc_as.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/mra.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54467
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "MCP RPC AS CHECKS FAILED"; then
...
echo "== checks: mcp_rpc_as_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: 15 functions added; 7 function bodies change =="
...
echo "== locks: no AccessExclusiveLock on any public table at the end of each apply =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  AE="$({ grep 'AccessExclusiveLock' "$WORK/locks_$k.txt" || true; } | cut -d= -f1 | sort | tr '\n' ',' | sed 's/,$//')"
  if [ -z "$AE" ]; then
    record "PASS[lock_${k}_apply_no_access_exclusive]: ${L:-no table locks}"
  else
    record "FAIL[lock_${k}_apply_no_access_exclusive]: AccessExclusiveLock on $AE ($L)"
  fi
done
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "MCP RPC AS CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL MCP RPC AS CHECKS PASSED (on real migrations)"
