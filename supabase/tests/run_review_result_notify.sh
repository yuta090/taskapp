#!/usr/bin/env bash
# =============================================================================
# 社内承認の「承認」「差し戻し」のお知らせ（*_review_result_notify.sql）の検証ハーネス
#
...
# 本 migration の前に review_result_notify_seed.sql（人・タスク）を入れ、本 migration を2回適用して（冪等）、
# review_result_notify_assert.sql で確かめる:
#   chg_*   本 migration で変わるもの（適用前は FAIL・適用後は PASS であるべき）
#   same_*  変えないもの（両方で PASS）
# 続けて GREEN のときだけ:
#   scope_*     本文が変わる関数は _review_approve_impl と _review_block_impl だけ。実行権・SECURITY DEFINER・
#               search_path・ほかの関数・トリガー・ポリシー・列・表の権限は変わらない
#   reapply_*   データが入った状態で再適用できる
#   guard_*     2つの関数のどちらかの今の定義が土台と違えば適用が止まり、何も変わらない
#   rollback_*  ファイル先頭のロールバック手順（20260912134823_mcp_rpc_as.sql の2つの create or replace を流す）で
#               土台の本文に戻り、戻したあと再適用できる
#
# 使い方:
#   bash supabase/tests/run_review_result_notify.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_review_result_notify.sh    # 本 migration を適用せずに流し、
...
#     （127.0.0.1 の TCP だけで待ち受ける。置き場所のパスが長くてもソケットの長さの上限に当たらない）
...
BASE_MIG="$MIG/20260912134823_mcp_rpc_as.sql"
RED="${RED:-0}"
PORT="${PORT:-55493}"
TMPBASE="${TMPBASE:-/tmp}"

# 土台（20260912134823_mcp_rpc_as.sql）の本文の md5
BASE_APPROVE_MD5='fd3140e13174335adce627b7a5e3b970'
BASE_BLOCK_MD5='cf8e7e8cc12b9bc563f34d8cc990ce9b'
APPROVE_SIG='public._review_approve_impl(uuid,uuid,uuid)'
BLOCK_SIG='public._review_block_impl(uuid,uuid,text,uuid)'

shopt -s nullglob
targets=("$MIG"/*_review_result_notify.sql)
shopt -u nullglob
...
mkdir -p "$TMPBASE"
WORK="$(mktemp -d "$TMPBASE/rrs.XXXXXX")"
PGDATA="$WORK/data"
...
#   _review_approve_impl / _review_block_impl だけは本文（md5）を除き、実行権・SECURITY DEFINER・search_path は見る
fingerprint(){
...
select x from (
  select 'fn ' || p.oid::regprocedure::text || ' definer=' || p.prosecdef::text
         ...
         || case when p.oid in ('public._review_approve_impl(uuid,uuid,uuid)'::regprocedure,
                                'public._review_block_impl(uuid,uuid,text,uuid)'::regprocedure)
...
apply base "$TST/review_result_notify_seed.sql"
...
echo "== checks: review_result_notify_assert.sql =="
OUT="$WORK/checks.out"
set +e
PGOPTIONS='--client-min-messages=notice' psql "$(conn checks)" -v ON_ERROR_STOP=1 \
  -f "$TST/review_result_notify_assert.sql" > "$OUT" 2>&1
set -e
...
echo "== scope: 本文が変わるのは _review_approve_impl と _review_block_impl だけ =="
fingerprint base > "$WORK/fp_after.txt"
if diff -q "$WORK/fp_before.txt" "$WORK/fp_after.txt" >/dev/null; then
  record "PASS[scope_only_two_bodies_changed]: same"
else
  record "FAIL[scope_only_two_bodies_changed]: $(diff "$WORK/fp_before.txt" "$WORK/fp_after.txt" | head -6 | tr '\n' ' ')"
fi
A_MD5="$(md5_of base "$APPROVE_SIG")"; B_MD5="$(md5_of base "$BLOCK_SIG")"
if [ "$A_MD5" != "$BASE_APPROVE_MD5" ] && [ "$B_MD5" != "$BASE_BLOCK_MD5" ] && [ "$A_MD5" != "none" ] && [ "$B_MD5" != "none" ]; then
  record "PASS[scope_two_bodies_replaced]: approve=$A_MD5 block=$B_MD5"
else
  record "FAIL[scope_two_bodies_replaced]: approve=$A_MD5 block=$B_MD5"
fi
...
echo "== guard: 2つの関数のどちらかの今の定義が土台と違えば止まる =="
guard_case(){
  ...
  a_before="$(md5_of "guard_$label" "$APPROVE_SIG")"; b_before="$(md5_of "guard_$label" "$BLOCK_SIG")"
  ...
  a_after="$(md5_of "guard_$label" "$APPROVE_SIG")"; b_after="$(md5_of "guard_$label" "$BLOCK_SIG")"
  if [ "$a_before" = "$a_after" ] && [ "$b_before" = "$b_after" ]; then
    record "PASS[guard_${label}_changes_nothing]: approve=$a_after block=$b_after"
  else
    record "FAIL[guard_${label}_changes_nothing]: approve $a_before -> $a_after, block $b_before -> $b_after"
  fi
}
guard_case approve "$APPROVE_SIG" "_review_approve_impl"
guard_case block "$BLOCK_SIG" "_review_block_impl"

echo "== rollback: 土台の2つの create or replace を流すと元の本文に戻り、そのあと再適用できる =="
newdb -T base rollback
RB="$WORK/rollback.sql"
awk '/^create or replace function public\._review_(approve|block)_impl\(/{f=1} f{print} f && /^\$\$;$/{f=0}' "$BASE_MIG" > "$RB"
fingerprint rollback > "$WORK/fp_rb_before.txt"
if apply rollback "$RB"; then
  A_RB="$(md5_of rollback "$APPROVE_SIG")"; B_RB="$(md5_of rollback "$BLOCK_SIG")"
  fingerprint rollback > "$WORK/fp_rb_after.txt"
  if [ "$A_RB" = "$BASE_APPROVE_MD5" ] && [ "$B_RB" = "$BASE_BLOCK_MD5" ] \
     && diff -q "$WORK/fp_rb_before.txt" "$WORK/fp_rb_after.txt" >/dev/null; then
    record "PASS[rollback_restores_base_bodies]: approve=$A_RB block=$B_RB"
  else
    record "FAIL[rollback_restores_base_bodies]: approve=$A_RB block=$B_RB $(diff "$WORK/fp_rb_before.txt" "$WORK/fp_rb_after.txt" | head -4 | tr '\n' ' ')"
  fi
  if apply rollback "$TARGET"; then
    record "PASS[rollback_then_reapply]: approve=$(md5_of rollback "$APPROVE_SIG") block=$(md5_of rollback "$BLOCK_SIG")"
  else
    record "FAIL[rollback_then_reapply]: apply failed"
  fi
else
  record "FAIL[rollback_restores_base_bodies]: rollback sql failed"
fi
...
if [ "$NFAIL" -ne 0 ] || [ "$NPASS" -eq 0 ]; then
  echo "REVIEW RESULT NOTIFY CHECKS FAILED"; exit 1
fi
echo "REVIEW RESULT NOTIFY CHECKS PASSED"
