#!/usr/bin/env bash
# =============================================================================
# 社内専用の新表2つ（*_internal_metrics_agency_tables.sql）の検証ハーネス
#   task_internal_metrics（タスクの実績工数）・space_agency_settings（space の代理店設定）
#
...
# 本 migration の前に internal_metrics_agency_tables_seed.sql（古い列に値のあるデータ）を入れる（埋め戻しを確かめるため）。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      internal_metrics_agency_tables_assert.sql（形・埋め戻し・役割ごとの読み書き・つなぎ・連鎖削除・二要素）
# 続けて GREEN のときだけ:
#   scope_*     本 migration で増えるのは新表2つ・その関数とトリガー・tasks と spaces のつなぎのトリガーだけで、既にある物は変わらない
...
#
# 使い方:
#   bash supabase/tests/run_internal_metrics_agency_tables.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_internal_metrics_agency_tables.sh    # 本 migration を適用せずに流し、
...
SEED="$TST/internal_metrics_agency_tables_seed.sql"
ASSERT="$TST/internal_metrics_agency_tables_assert.sql"
RED="${RED:-0}"
# 本 migration で増える物の名前（scope_* で「これ以外は変わらない」を見る）
NEW_RE='task_internal_metrics|space_agency_settings|bridge_task_actual_hours|bridge_space_agency_settings'

shopt -s nullglob
targets=("$MIG"/*_internal_metrics_agency_tables.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/ima.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54459
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "INTERNAL METRICS AGENCY TABLES CHECKS FAILED"; then
...
echo "== seed (values in the old columns, before the target) =="
apply base "$SEED"
...
echo "== checks: internal_metrics_agency_tables_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: only new objects are added; nothing that existed changes =="
...
NADD="$(printf '%s\n' "$ADDED" | grep -c . || true)"
if [ -z "$REMOVED" ] && [ -z "$OUTSIDE" ] && [ "$NADD" -gt 0 ]; then
  record "PASS[scope_only_new_objects_added]: $NADD lines added (new tables / functions / triggers), nothing removed or changed"
else
  record "FAIL[scope_only_new_objects_added]: removed/changed: $(printf '%s' "$REMOVED" | head -3 | tr '\n' ';') other added: $(printf '%s' "$OUTSIDE" | head -3 | tr '\n' ';')"
fi
...
if [ "$NBLK" -eq 3 ]; then
  ...
  record "FAIL[rollback_sections_found]: $NBLK sections, $NSTMT statements (want 3 sections)"
fi
...
# (2) 挙動も戻る: 戻した複製で assert を流すと RED と同じ形 → 再適用した複製では全 PASS（埋め戻しも古い列からやり直す）
newdb -T base rb
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "INTERNAL METRICS AGENCY TABLES CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL INTERNAL METRICS AGENCY TABLES CHECKS PASSED (on real migrations)"
