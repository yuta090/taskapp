#!/usr/bin/env bash
# =============================================================================
# タスクごとのコメント数の RPC（*_task_comment_counts.sql）の検証ハーネス
#
...
# migration は1行も変えない。
# 本 migration の前に task_comment_counts_seed.sql でタスクとコメントを作り、本 migration を2回適用して（冪等）、
# task_comment_counts_assert.sql で確かめる:
#   chg_*   本 migration で変わるもの（適用前は FAIL・適用後は PASS であるべき）
#   same_*  変えないもの（両方で PASS）
# 続けて GREEN のときだけ:
#   scope_*     本 migration で増えるのは rpc_task_comment_counts だけ。ほかの関数・ポリシー・列・表の権限・索引は変わらない
#   reapply_*   データが入った状態で再適用でき、数も変わらない
#   rollback_*  ファイル先頭のロールバックを流すと本 migration の前の形に戻り、そのあと再適用できる
#
# 使い方:
#   bash supabase/tests/run_task_comment_counts.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_task_comment_counts.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#       （本 migration のファイルがまだ無いときは、今ある migrations を全部流す）
# 一時フォルダの置き場所は HARNESS_TMP で変えられる（既定は /tmp）。接続は 127.0.0.1 の TCP で、
# 置き場所のパスが長くてもソケットの長さの上限に当たらない。
...
shopt -s nullglob
targets=("$MIG"/*_task_comment_counts.sql)
shopt -u nullglob
...
WORK="$(mktemp -d "${HARNESS_TMP:-/tmp}/tcc.XXXXXX")"
PGDATA="$WORK/data"; PORT="${PORT:-54472}"; HOST=127.0.0.1
...
pg_ctl -D "$PGDATA" -l "$WORK/pg.log" -o "-p $PORT -c listen_addresses=$HOST -c unix_socket_directories=''" -w start >/dev/null 2>&1
...
select x from (
  select 'fn ' || p.oid::regprocedure::text || ' definer=' || p.prosecdef::text || ' volatile=' || p.provolatile::text
...
echo "== seed: タスクとコメント（本 migration の前からあるデータ） =="
apply base "$TST/task_comment_counts_seed.sql"

if [ "$RED" = "1" ]; then
  echo "== RED mode: target migration is NOT applied =="
else
  # 本 migration の前の状態の指紋を残しておく（scope_* / rollback_* 用）
...
echo "== checks: task_comment_counts_assert.sql =="
OUT="$WORK/checks.out"
set +e
PGOPTIONS='--client-min-messages=notice' psql "$(conn checks)" -v ON_ERROR_STOP=1 \
  -f "$TST/task_comment_counts_assert.sql" > "$OUT" 2>&1
set -e
# grep は一致が無いと 1 を返す（pipefail で止まらないよう || true）
grep -oE "(PASS|FAIL)\[[a-z0-9_]+\].*" "$OUT" >> "$RES" || true

# assert の中のエラーは test.q_as が文字列にして返すので、ここに出る ERROR はハーネスかテストデータの不備
...
echo "== scope: 増えるのは rpc_task_comment_counts だけ =="
fingerprint base | grep -v 'rpc_task_comment_counts' > "$WORK/fp_after.txt"
if diff -q "$WORK/fp_before.txt" "$WORK/fp_after.txt" >/dev/null; then
  record "PASS[scope_only_rpc_added]: same"
else
  record "FAIL[scope_only_rpc_added]: $(diff "$WORK/fp_before.txt" "$WORK/fp_after.txt" | head -5 | tr '\n' ' ')"
fi
N_FN="$(q base "select count(*) from pg_proc where proname = 'rpc_task_comment_counts'")"
if [ "$N_FN" = "1" ]; then record "PASS[scope_single_overload]: $N_FN"; else record "FAIL[scope_single_overload]: $N_FN"; fi

# 社内の人として S1 の数を読む（再適用・ロールバックの前後で比べる）
S1_SQL="select test.rpc_as('authenticated', 'c1000000-0000-0000-0000-000000000001'::uuid, \$a\$p_space_id => 'b1000000-0000-0000-0000-000000000001'\$a\$)"

echo "== re-apply with data present =="
C_BEFORE="$(q checks "$S1_SQL")"
if apply checks "$TARGET"; then record "PASS[reapply_with_data]: ok"; else record "FAIL[reapply_with_data]: apply failed"; fi
C_AFTER="$(q checks "$S1_SQL")"
if [ "$C_BEFORE" = "T1=7,T2=3,T3=3,T8=3" ] && [ "$C_BEFORE" = "$C_AFTER" ]; then
  record "PASS[reapply_same_counts]: $C_AFTER"
else
  record "FAIL[reapply_same_counts]: $C_BEFORE -> $C_AFTER"
fi

echo "== rollback: ファイル先頭のロールバックで本 migration の前の形に戻る =="
newdb -T base rb
q rb "drop function if exists public.rpc_task_comment_counts(uuid, uuid, uuid);" >/dev/null
fingerprint rb > "$WORK/fp_rb.txt"
if diff -q "$WORK/fp_before.txt" "$WORK/fp_rb.txt" >/dev/null; then
  record "PASS[rollback_restores_schema]: same"
else
  record "FAIL[rollback_restores_schema]: $(diff "$WORK/fp_before.txt" "$WORK/fp_rb.txt" | head -5 | tr '\n' ' ')"
fi
if apply rb "$TARGET"; then record "PASS[rollback_then_reapply]: ok"; else record "FAIL[rollback_then_reapply]: apply failed"; fi
...
if [ "$NFAIL" -ne 0 ] || [ "$NPASS" -eq 0 ]; then
  echo "TASK COMMENT COUNTS CHECKS FAILED"; exit 1
fi
echo "TASK COMMENT COUNTS CHECKS PASSED"
