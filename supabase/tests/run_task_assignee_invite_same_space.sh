#!/usr/bin/env bash
# =============================================================================
# タスクの担当の招待は同じ space の招待だけ（*_task_assignee_invite_same_space.sql）の検証ハーネス
#
...
# 本 migration の前に task_assignee_invite_same_space_seed.sql（2つの組織・3つの space・招待4つ・タスク4つ）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      task_assignee_invite_same_space_assert.sql（同じ space の招待は置ける・別の space / 組織の招待は止まる
#               〈postgres・service role・画面の編集者〉・担当を空にする / 本人に替える / 招待を消す / 招待の承諾はそのまま・形）
# 続けて GREEN のときだけ:
#   scope_*     増えるのはトリガー関数1つとトリガー1つだけ（ほかは何も変わらない）
#   reapply_*   データが入った状態で再適用できる
#   rollback_*  ロールバック節を流すと、適用前のスキーマと挙動に戻る → 戻したあと再適用できる
#   lock_*      本 migration を流した（初回と2回目の）トランザクションの最後に、同じ接続が tasks を share row exclusive で
#               持ち、public の表に AccessExclusiveLock を持っていない（読みは止めない）
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_task_assignee_invite_same_space.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_task_assignee_invite_same_space.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_task_assignee_invite_same_space.sh
...
SEED="$TST/task_assignee_invite_same_space_seed.sql"
ASSERT="$TST/task_assignee_invite_same_space_assert.sql"
RED="${RED:-0}"
# 増える物（トリガー関数1つとトリガー1つ）。変わる物は無い
NEW_RE='enforce_assignee_invite_same_space'
CHANGED_RE='^$'
WANT_ADDED=2
WANT_REMOVED=0
# ロールバック節の数と、; で終わる行の数（節 1）
WANT_RB_BLOCKS=1
WANT_RB_STMTS=2

shopt -s nullglob
targets=("$MIG"/*_task_assignee_invite_same_space.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/tai.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54468
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "TASK INVITE SAME SPACE CHECKS FAILED"; then
...
echo "== checks: task_assignee_invite_same_space_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: 1 trigger function and 1 trigger added; nothing else changes =="
...
echo "== locks: tasks in share row exclusive and no AccessExclusiveLock on any public table at the end of each apply =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  AE="$({ grep 'AccessExclusiveLock' "$WORK/locks_$k.txt" || true; } | cut -d= -f1 | sort | tr '\n' ',' | sed 's/,$//')"
  if [ -z "$AE" ] && grep -q '^tasks=.*ShareRowExclusiveLock' "$WORK/locks_$k.txt"; then
    record "PASS[lock_${k}_apply_share_row_exclusive_on_tasks_no_access_exclusive]: $L"
  else
    record "FAIL[lock_${k}_apply_share_row_exclusive_on_tasks_no_access_exclusive]: AccessExclusiveLock on ${AE:-none} ($L)"
  fi
done
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "TASK INVITE SAME SPACE CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL TASK INVITE SAME SPACE CHECKS PASSED (on real migrations)"
