#!/usr/bin/env bash
# =============================================================================
# space と組織の境目を DB でも守る（*_space_scope_invariants.sql）の検証ハーネス
#
...
# 本 migration の前に space_scope_invariants_seed.sql（既にある行）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      space_scope_invariants_assert.sql（形・タスクの Wiki ページ・招待・space のメンバー・mcp_authorize）
# 続けて GREEN のときだけ:
#   scope_*     増えるのは関数2つ・トリガー2本・外部キー1本で、変わるのは mcp_authorize の本文だけ
#   reapply_*   データが入った状態で再適用できる
#   rollback_*  ロールバック節を流すと、適用前のスキーマと挙動に戻る → 戻したあと再適用できる
#   lock_*      本 migration を流した（初回と2回目の）トランザクションの最後に、同じ接続が tasks・spaces・invites・
#               space_memberships に AccessExclusiveLock を持っていない（pg_locks・pg_backend_pid()。読みを止めない）
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_space_scope_invariants.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_space_scope_invariants.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_space_scope_invariants.sh
...
SEED="$TST/space_scope_invariants_seed.sql"
ASSERT="$TST/space_scope_invariants_assert.sql"
RED="${RED:-0}"
# 増える物（関数2つ・トリガー2本・外部キー1本）と、本文が変わる mcp_authorize
NEW_RE='enforce_wiki_page_same_space|enforce_space_member_in_org|invites_space_org_fkey|^fn mcp_authorize\('
CHANGED_RE='^fn mcp_authorize\('
WANT_ADDED=6
WANT_REMOVED=1
# ロールバック節の数と文の数（節 1〜4）
WANT_RB_BLOCKS=4
...
shopt -s nullglob
targets=("$MIG"/*_space_scope_invariants.sql)
shopt -u nullglob
...
#   （tasks・spaces・invites・space_memberships。1行 = 表=ロックの種類+…）
...
select c.relname || '=' || string_agg(l.mode, '+' order by l.mode)
 ...
 where l.pid = pg_backend_pid() and l.locktype = 'relation' and l.granted
   ...
   and c.relname in ('tasks', 'spaces', 'invites', 'space_memberships')
 group by c.relname
 order by c.relname;
SQL
  } > "$w"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "SPACE SCOPE INVARIANTS CHECKS FAILED"; then
...
echo "== checks: space_scope_invariants_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: 2 functions, 2 triggers and 1 foreign key are added; only mcp_authorize's body changes =="
...
if [ -z "$OUT_ADDED" ] && [ -z "$OUT_REMOVED" ] && [ "$NADD" -eq "$WANT_ADDED" ] && [ "$NREM" -eq "$WANT_REMOVED" ]; then
  record "PASS[scope_only_intended_objects]: $NADD lines added, $NREM line replaced (mcp_authorize body), nothing else changed"
else
...
echo "== locks: no AccessExclusiveLock on tasks / spaces / invites / space_memberships at the end of each apply =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  if grep -q 'AccessExclusiveLock' "$WORK/locks_$k.txt"; then
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "SPACE SCOPE INVARIANTS CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL SPACE SCOPE INVARIANTS CHECKS PASSED (on real migrations)"
