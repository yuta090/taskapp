#!/usr/bin/env bash
# =============================================================================
# マイルストーンの並び順は bigint（*_milestones_order_key_bigint.sql）の検証ハーネス
#
...
# 本 migration の前に milestones_order_key_bigint_seed.sql（既にあるマイルストーン）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      milestones_order_key_bigint_assert.sql（型・ミリ秒と秒の order_key・並び・既にある行・ビュー・プリセットの RPC）
# 続けて GREEN のときだけ:
#   scope_*     変わるのは milestones.order_key の型だけ
...
#
# 使い方:
#   bash supabase/tests/run_milestones_order_key_bigint.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_milestones_order_key_bigint.sh    # 本 migration を適用せずに流し、
...
SEED="$TST/milestones_order_key_bigint_seed.sql"
ASSERT="$TST/milestones_order_key_bigint_assert.sql"
RED="${RED:-0}"
# 変わるのは milestones.order_key の型だけ（指紋の1行が入れ替わる）
NEW_RE='^col milestones\.order_key bigint '
CHANGED_RE='^col milestones\.order_key integer '
WANT_ADDED=1
WANT_REMOVED=1
# ロールバック節の数と文の数（節 1）
WANT_RB_BLOCKS=1
...
shopt -s nullglob
targets=("$MIG"/*_milestones_order_key_bigint.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/mok.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54462
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "MILESTONES ORDER KEY BIGINT CHECKS FAILED"; then
...
echo "== seed (milestones that already exist, before the target) =="
apply base "$SEED"
...
echo "== checks: milestones_order_key_bigint_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: only the type of milestones.order_key changes =="
...
if [ -z "$OUT_ADDED" ] && [ -z "$OUT_REMOVED" ] && [ "$NADD" -eq "$WANT_ADDED" ] && [ "$NREM" -eq "$WANT_REMOVED" ]; then
  record "PASS[scope_only_order_key_type]: milestones.order_key integer -> bigint, nothing else changed"
else
  record "FAIL[scope_only_order_key_type]: added $NADD (want $WANT_ADDED), removed $NREM (want $WANT_REMOVED); other added: $(printf '%s' "$OUT_ADDED" | head -3 | tr '\n' ';') other removed: $(printf '%s' "$OUT_REMOVED" | head -3 | tr '\n' ';')"
fi
...
if [ "$NBLK" -eq "$WANT_RB_BLOCKS" ] && [ "$NSTMT" -eq "$WANT_RB_STMTS" ]; then
  record "PASS[rollback_sections_found]: $NBLK section, $NSTMT statement"
else
  record "FAIL[rollback_sections_found]: $NBLK sections, $NSTMT statements (want $WANT_RB_BLOCKS sections, $WANT_RB_STMTS statements)"
fi
...
# (3) 収まらない値があるとロールバックは止まる（ミリ秒の order_key を1行入れてから戻す → 22003）
newdb -T base rb3
apply rb3 "$TARGET"
psql "$(conn rb3)" -q -v ON_ERROR_STOP=1 -c "update public.milestones set order_key = 1757649600000 where id = '00000000-0000-0000-0000-00000000e003'" >/dev/null
if PGOPTIONS='--client-min-messages=warning' psql "$(conn rb3)" -q -v ON_ERROR_STOP=1 -1 -f "$RB" > "$WORK/rollback_rb3.out" 2>&1; then
  record "FAIL[rollback_stops_when_values_do_not_fit]: rollback went through with a value outside integer"
elif grep -q "out of range" "$WORK/rollback_rb3.out"; then
  record "PASS[rollback_stops_when_values_do_not_fit]: $(grep -m1 -o 'integer out of range' "$WORK/rollback_rb3.out")"
else
  record "FAIL[rollback_stops_when_values_do_not_fit]: $(head -3 "$WORK/rollback_rb3.out" | tr '\n' ';')"
fi
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "MILESTONES ORDER KEY BIGINT CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL MILESTONES ORDER KEY BIGINT CHECKS PASSED (on real migrations)"
