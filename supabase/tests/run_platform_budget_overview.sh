#!/usr/bin/env bash
# =============================================================================
# 共有bot(共通LINE) グローバル予算層: 運用者向け可視化関数 app_platform_budget_overview() 検証ハーネス
#
# prior migrations＋予算層2本＋20260720201116_platform_budget_overview.sql を1行も改変せず
# verbatim 適用し、platform_budget_overview_data.sql で内容・並び・権限を検証する
# （run_platform_channel_budget.sh と同型・手コピー禁止）。使い捨てクラスタを起動し終了時に破棄。
#
# 使い方: bash supabase/tests/run_platform_budget_overview.sh
...
WORK="$(mktemp -d /tmp/pbo.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54423
mkdir -p "$SOCK"
...
echo "== budget layer migrations (verbatim) =="
...
OUT="$WORK/o.out"
PGOPTIONS='--client-min-messages=notice' psql "$CONN" -v ON_ERROR_STOP=1 -f "$TST/platform_budget_overview_data.sql" > "$OUT" 2>&1 \
...
grep -q "PLATFORM BUDGET OVERVIEW CHECKS PASSED" "$OUT" || { echo "NOT PASSED"; tail -30 "$OUT"; exit 1; }
echo ""
echo "ALL PLATFORM BUDGET OVERVIEW CHECKS PASSED (on real migrations)"
