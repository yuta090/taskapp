...
# 「確定した時点の控え」の検証ハーネス
#
# 空DBに全 migration を順に適用してから spec_state_as_assert.sql を流す。
...
echo "== CLIからの決定の検証 =="
OUT="$WORK/o.out"
PGOPTIONS='--client-min-messages=notice' psql "$CONN" -v ON_ERROR_STOP=1 -f "$TST/spec_state_as_assert.sql" > "$OUT" 2>&1 \
  || { echo "STEP FAILED"; grep -E "ERROR|例外" "$OUT" | head; tail -20 "$OUT"; exit 1; }
grep -oE "PASS [0-9]+\)[^\\\\]*" "$OUT" || true
grep -q "CLIからの決定 全項目 PASS" "$OUT" || { echo "NOT PASSED"; tail -30 "$OUT"; exit 1; }
echo ""
echo "ALL SPEC STATE AS CHECKS PASSED (on real migrations)"
