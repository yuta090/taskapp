#!/usr/bin/env bash
# =============================================================================
# メンバー情報が見える範囲（*_member_directory_visibility.sql）の検証ハーネス
#
...
# 本 migration の前に member_directory_visibility_seed.sql（視点ごとの人・招待）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      member_directory_visibility_assert.sql（形・profiles・rpc_get_org_members・invites・org_memberships）
# 続けて GREEN のときだけ:
#   scope_*     増えるのは関数1つ・索引2つ・列の権限で、変わるのは3表の読み取りのポリシー・2表の権限・rpc_get_org_members の本文だけ
...
#               持っている表が profiles・invites・org_memberships だけ（space_memberships は share まで）
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_member_directory_visibility.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_member_directory_visibility.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_member_directory_visibility.sh
...
SEED="$TST/member_directory_visibility_seed.sql"
ASSERT="$TST/member_directory_visibility_assert.sql"
RED="${RED:-0}"
# 増える物（関数1つ・索引2つ・invites の列の権限）と、変わる物（3表の読み取りのポリシー・profiles と invites の表の権限・
#   rpc_get_org_members の本文）
NEW_RE='app_can_see_profile|_user_id_idx|^colacl invites\.|^pol (profiles|invites|org_memberships) |^rel (profiles|invites) |rpc_get_org_members'
CHANGED_RE='^pol (profiles|invites|org_memberships) |^rel (profiles|invites) |rpc_get_org_members'
WANT_ADDED=20
WANT_REMOVED=6
# ロールバック節の数と、; で終わる行の数（節 1〜4。関数の本文の行も数える）
WANT_RB_BLOCKS=4
WANT_RB_STMTS=22

shopt -s nullglob
targets=("$MIG"/*_member_directory_visibility.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/mdv.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54465
mkdir -p "$SOCK"
...
#   （profiles・invites・org_memberships・space_memberships。1行 = 表=ロックの種類+…）
...
select c.relname || '=' || string_agg(l.mode, '+' order by l.mode)
 ...
 where l.pid = pg_backend_pid() and l.locktype = 'relation' and l.granted
   ...
   and c.relname in ('profiles', 'invites', 'org_memberships', 'space_memberships')
 group by c.relname
 order by c.relname;
SQL
  } > "$w"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "MEMBER DIRECTORY CHECKS FAILED"; then
...
echo "== checks: member_directory_visibility_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: 1 function, 2 indexes and invites column grants added; 3 read policies, 2 table grants and 1 function body change =="
...
echo "== locks: AccessExclusiveLock only on profiles / invites / org_memberships at the end of each apply =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  AE="$(grep 'AccessExclusiveLock' "$WORK/locks_$k.txt" | cut -d= -f1 | sort | tr '\n' ',' | sed 's/,$//')"
  if [ "$AE" = "invites,org_memberships,profiles" ]; then
    record "PASS[lock_${k}_apply_access_exclusive_only_on_3_tables]: $L"
  else
    record "FAIL[lock_${k}_apply_access_exclusive_only_on_3_tables]: AccessExclusiveLock on ${AE:-none} ($L)"
  fi
done
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "MEMBER DIRECTORY CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL MEMBER DIRECTORY CHECKS PASSED (on real migrations)"
