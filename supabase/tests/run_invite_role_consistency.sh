#!/usr/bin/env bash
# =============================================================================
# 招待は組織の役割と合う種類だけ（*_invite_role_consistency.sql）の検証ハーネス
#
...
# 本 migration の前に invite_role_consistency_seed.sql（2つの組織・代理店モードの space・参加済みの人・
# 承諾待ち / 期限切れ / 承諾済みの招待）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      invite_role_consistency_assert.sql（合わない招待は作れない〈postgres・service role・まとめて〉・
#               再送で生き返らない・承諾 / 期限切れ / 取り消しは今までどおり・rpc_create_invite の使い回しと順番・形）
# 続けて GREEN のときだけ:
#   scope_*     増えるのはトリガー関数1つとトリガー1つ、変わるのは rpc_create_invite の本文だけ
#   reapply_*   データが入った状態で再適用できる
#   rollback_*  ロールバック節を流すと、適用前のスキーマと挙動に戻る → 戻したあと再適用できる
#   lock_*      本 migration を流した（初回と2回目の）トランザクションの最後に、同じ接続が invites を
#               share row exclusive で持ち、public の表に AccessExclusiveLock を持っていない（読みは止めない）
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_invite_role_consistency.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_invite_role_consistency.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_invite_role_consistency.sh
...
SEED="$TST/invite_role_consistency_seed.sql"
ASSERT="$TST/invite_role_consistency_assert.sql"
RED="${RED:-0}"
# 増える物（トリガー関数1つとトリガー1つ）と、変わる物（rpc_create_invite の本文）
NEW_RE='enforce_invite_role_consistency|^fn rpc_create_invite\('
CHANGED_RE='^fn rpc_create_invite\('
WANT_ADDED=3
WANT_REMOVED=1
# ロールバック節の数と、; で終わる行の数（節 1〜2。関数の本文の行も数える）
WANT_RB_BLOCKS=2
WANT_RB_STMTS=41

shopt -s nullglob
targets=("$MIG"/*_invite_role_consistency.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/irc.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54469
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "INVITE ROLE CHECKS FAILED"; then
...
echo "== checks: invite_role_consistency_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: 1 trigger function and 1 trigger added; 1 function body changes =="
...
echo "== locks: invites in share row exclusive and no AccessExclusiveLock on any public table at the end of each apply =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  AE="$({ grep 'AccessExclusiveLock' "$WORK/locks_$k.txt" || true; } | cut -d= -f1 | sort | tr '\n' ',' | sed 's/,$//')"
  if [ -z "$AE" ] && grep -q '^invites=.*ShareRowExclusiveLock' "$WORK/locks_$k.txt"; then
    record "PASS[lock_${k}_apply_share_row_exclusive_on_invites_no_access_exclusive]: $L"
  else
    record "FAIL[lock_${k}_apply_share_row_exclusive_on_invites_no_access_exclusive]: AccessExclusiveLock on ${AE:-none} ($L)"
  fi
done
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "INVITE ROLE CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL INVITE ROLE CHECKS PASSED (on real migrations)"
