...
# 「確定した時点の控え」の検証ハーネス
#
# 空DBに全 migration を順に適用してから wiki_decision_versions_assert.sql を流す。
...
WORK="$(mktemp -d /tmp/wdv.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54431
mkdir -p "$SOCK"
...
echo "== 確定時点の控えの検証 =="
OUT="$WORK/o.out"
PGOPTIONS='--client-min-messages=notice' psql "$CONN" -v ON_ERROR_STOP=1 -f "$TST/wiki_decision_versions_assert.sql" > "$OUT" 2>&1 \
  || { echo "STEP FAILED"; grep -E "ERROR|例外" "$OUT" | head; tail -20 "$OUT"; exit 1; }
grep -oE "PASS [0-9]+\)[^\\\\]*" "$OUT" || true
grep -q "確定時点の控え 全項目 PASS" "$OUT" || { echo "NOT PASSED"; tail -30 "$OUT"; exit 1; }
echo ""
echo "ALL WIKI DECISION VERSION CHECKS PASSED (on real migrations)"
