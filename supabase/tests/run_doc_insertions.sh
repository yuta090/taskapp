#!/usr/bin/env bash
# =============================================================================
# 相手先の差し込み（doc_insertions）の検証ハーネス
#
# 空DBに全 migration を順に適用してから doc_insertions_assert.sql を流す。
...
RED="${RED:-0}"
TARGET="doc_insertions.sql"

WORK="$(mktemp -d /tmp/dins.XXXXXX)"
...
echo "== 投票ブロックの検証 =="
OUT="$WORK/o.out"
if ! PGOPTIONS='--client-min-messages=notice' psql "$CONN" -v ON_ERROR_STOP=1 -f "$TST/doc_insertions_assert.sql" > "$OUT" 2>&1; then
  grep -oE "PASS\[[^]]*\]" "$OUT" | tr '\n' ' ' || true
  echo ""; echo "STEP FAILED"; grep -E "ERROR|FAIL" "$OUT" | head -5
  exit 1
fi
grep -c "PASS\[" "$OUT" | xargs -I{} echo "-- PASS {} 件"
grep -q "DOC INSERTIONS 全項目 PASS" "$OUT" || { echo "NOT PASSED"; tail -30 "$OUT"; exit 1; }
echo "ALL DOC INSERTIONS CHECKS PASSED (on real migrations)"
