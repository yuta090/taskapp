#!/usr/bin/env bash
# =============================================================================
# 組織の役割と space の役割をそろえる（*_org_space_role_consistency.sql）の検証ハーネス
#
...
# 本 migration の前に org_space_role_consistency_seed.sql（既にある行）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      org_space_role_consistency_assert.sql（形・space の役割・2つの RPC・commit の時点の確かめ・招待・seed・task_pricing）
# 続けて GREEN のときだけ:
#   scope_*     増えるのは関数1つ・制約トリガー1本で、変わるのは関数3つの本文・space のトリガーの列・task_pricing の4ポリシーだけ
#   reapply_*   データが入った状態で再適用できる
#   rollback_*  ロールバック節を流すと、適用前のスキーマと挙動に戻る → 戻したあと再適用できる
#   lock_*      本 migration を流した（初回と2回目の）トランザクションの最後に、同じ接続が持っている表のロックを出し、
#               org_memberships・space_memberships に AccessExclusiveLock が無いことを確かめる（読みを止めない）。
#               task_pricing はポリシーを書き換えるので AccessExclusiveLock を持つ（手元の結果。本番のドライランで確かめる）
#   commit_*    本当の commit で: 組織の役割だけを書き換えると commit で止まり、何も変わらない／
#               rpc_update_org_member_role は組織と space を1トランザクションでそろえて commit できる
#
# assert の label:
...
#
# 使い方:
#   bash supabase/tests/run_org_space_role_consistency.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_org_space_role_consistency.sh    # 本 migration を適用せずに流し、
#       chg_* が全て FAIL・same_* が全て PASS する（= テストが変化を検出でき、変えない所は従来どおり）ことを確認する
#   TARGET_FILE=<file> bash supabase/tests/run_org_space_role_consistency.sh
#       本 migration の位置で別の版を流す（検査が効くことを確かめる用。例: drop trigger を使う版では lock_* が FAIL する）
...
SEED="$TST/org_space_role_consistency_seed.sql"
ASSERT="$TST/org_space_role_consistency_assert.sql"
RED="${RED:-0}"
# 増える物（関数1つ・制約トリガー1本とその制約）と、変わる物（関数3つの本文・space のトリガー・task_pricing の4ポリシー）
NEW_RE='enforce_space_member_in_org|enforce_org_member_role_matches_spaces|trg_org_member_role_matches_spaces|rpc_update_org_member_role|rpc_update_space_member_role|^pol task_pricing '
CHANGED_RE='enforce_space_member_in_org|rpc_update_org_member_role|rpc_update_space_member_role|^pol task_pricing '
WANT_ADDED=11
WANT_REMOVED=8
# ロールバック節の数と、; で終わる行の数（節 1〜5。関数の本文の行も数える）
WANT_RB_BLOCKS=5
WANT_RB_STMTS=56

shopt -s nullglob
targets=("$MIG"/*_org_space_role_consistency.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/rcs.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54463
mkdir -p "$SOCK"
...
#   （org_memberships・space_memberships・task_pricing・spaces。1行 = 表=ロックの種類+…）
...
select c.relname || '=' || string_agg(l.mode, '+' order by l.mode)
 ...
 where l.pid = pg_backend_pid() and l.locktype = 'relation' and l.granted
   ...
   and c.relname in ('org_memberships', 'space_memberships', 'task_pricing', 'spaces')
 group by c.relname
 order by c.relname;
SQL
  } > "$w"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "ORG SPACE ROLE CONSISTENCY CHECKS FAILED"; then
...
echo "== checks: org_space_role_consistency_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: 1 function and 1 constraint trigger added; 3 function bodies, the space trigger and 4 task_pricing policies change =="
...
echo "== locks: no AccessExclusiveLock on org_memberships / space_memberships at the end of each apply =="
for k in first second; do
  L="$(tr '\n' ' ' < "$WORK/locks_$k.txt" | sed 's/ *$//')"
  if grep -E '^(org_memberships|space_memberships)=' "$WORK/locks_$k.txt" | grep -q 'AccessExclusiveLock'; then
    record "FAIL[lock_${k}_apply_no_access_exclusive]: $L"
  elif [ -s "$WORK/locks_$k.txt" ]; then
    record "PASS[lock_${k}_apply_no_access_exclusive]: $L"
  else
    record "FAIL[lock_${k}_apply_no_access_exclusive]: no lock rows (the probe did not run in the same transaction)"
  fi
done

echo "== commit-time checks (real commits on a copy) =="
newdb -T base cm
set +e
OUT_A="$(psql "$(conn cm)" -v ON_ERROR_STOP=1 -q 2>&1 <<'SQL'
begin;
update public.org_memberships set role = 'client'
 where org_id = '00000000-0000-0000-0000-00000000a001' and user_id = '00000000-0000-0000-0000-00000000c002';
commit;
SQL
)"
RC_A=$?
set -e
ROLE_A="$(psql "$(conn cm)" -qtA -c "select role from public.org_memberships where org_id = '00000000-0000-0000-0000-00000000a001' and user_id = '00000000-0000-0000-0000-00000000c002'")"
if [ "$RC_A" -ne 0 ] && printf '%s' "$OUT_A" | grep -q 'does not match space roles' && [ "$ROLE_A" = "member" ]; then
  record "PASS[commit_rejects_org_role_without_space_fix]: $(printf '%s' "$OUT_A" | grep -m1 -o 'organization role .*') / role stays $ROLE_A"
else
  record "FAIL[commit_rejects_org_role_without_space_fix]: rc=$RC_A role=$ROLE_A out=$(printf '%s' "$OUT_A" | head -3 | tr '\n' ';')"
fi
set +e
OUT_B="$(psql "$(conn cm)" -v ON_ERROR_STOP=1 -q 2>&1 <<'SQL'
begin;
set local role authenticated;
select set_config('request.jwt.claims', '{"sub":"00000000-0000-0000-0000-00000000c001","role":"authenticated"}', true);
select public.rpc_update_org_member_role('00000000-0000-0000-0000-00000000a001', '00000000-0000-0000-0000-00000000c003', 'member');
commit;
SQL
)"
RC_B=$?
set -e
ROLES_B="$(psql "$(conn cm)" -qtA -c "select (select role from public.org_memberships where org_id = '00000000-0000-0000-0000-00000000a001' and user_id = '00000000-0000-0000-0000-00000000c003') || '/' || (select role from public.space_memberships where space_id = '00000000-0000-0000-0000-00000000b001' and user_id = '00000000-0000-0000-0000-00000000c003')")"
if [ "$RC_B" -eq 0 ] && [ "$ROLES_B" = "member/viewer" ]; then
  record "PASS[commit_org_rpc_aligns_spaces]: committed, org/space = $ROLES_B"
else
  record "FAIL[commit_org_rpc_aligns_spaces]: rc=$RC_B roles=$ROLES_B out=$(printf '%s' "$OUT_B" | head -3 | tr '\n' ';')"
fi
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "ORG SPACE ROLE CONSISTENCY CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL ORG SPACE ROLE CONSISTENCY CHECKS PASSED (on real migrations)"
