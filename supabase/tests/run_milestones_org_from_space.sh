#!/usr/bin/env bash
# =============================================================================
# マイルストーンの組織は、その space の組織から決める（*_milestones_org_from_space.sql）の検証ハーネス
#
...
# 本 migration の前に milestones_org_from_space_seed.sql（既にあるマイルストーン）を入れる。
# そのうえで本 migration を2回適用し（冪等）、複製で検証する:
#   checks      milestones_org_from_space_assert.sql（形・作る・変える・RLS・既にある行）
# 続けて GREEN のときだけ:
#   scope_*     本 migration で増えるのは関数とトリガーだけで、既にある物は変わらない
...
#
# 使い方:
#   bash supabase/tests/run_milestones_org_from_space.sh          # 全 PASS を期待
#   RED=1 bash supabase/tests/run_milestones_org_from_space.sh    # 本 migration を適用せずに流し、
...
SEED="$TST/milestones_org_from_space_seed.sql"
ASSERT="$TST/milestones_org_from_space_assert.sql"
RED="${RED:-0}"
NEW_RE='milestones_fill_org'

shopt -s nullglob
targets=("$MIG"/*_milestones_org_from_space.sql)
shopt -u nullglob
...
WORK="$(mktemp -d /tmp/mof.XXXXXX)"
PGDATA="$WORK/data"; SOCK="$WORK/s"; PORT=54460
mkdir -p "$SOCK"
...
run_assert(){
  ...
  set -e
  if grep "ERROR" "$2" | grep -qv "MILESTONES ORG FROM SPACE CHECKS FAILED"; then
...
echo "== seed (a milestone that already exists, before the target) =="
apply base "$SEED"
...
echo "== checks: milestones_org_from_space_assert.sql =="
OUT="$WORK/checks.out"
...
echo "== scope: only the new function and trigger are added; nothing that existed changes =="
...
NADD="$(printf '%s\n' "$ADDED" | grep -c . || true)"
if [ -z "$REMOVED" ] && [ -z "$OUTSIDE" ] && [ "$NADD" -eq 2 ]; then
  record "PASS[scope_only_new_objects_added]: function and trigger added (2 lines), nothing removed or changed"
else
  record "FAIL[scope_only_new_objects_added]: $NADD lines added; removed/changed: $(printf '%s' "$REMOVED" | head -3 | tr '\n' ';') other added: $(printf '%s' "$OUTSIDE" | head -3 | tr '\n' ';')"
fi
...
if [ "$NBLK" -eq 1 ] && [ "$NSTMT" -eq 2 ]; then
  record "PASS[rollback_sections_found]: $NBLK section, $NSTMT statements"
else
  record "FAIL[rollback_sections_found]: $NBLK sections, $NSTMT statements (want 1 section, 2 statements)"
fi
...
if [ "$NFAIL" -ne 0 ] || ! grep -q "MILESTONES ORG FROM SPACE CHECKS PASSED" "$OUT"; then
  echo "NOT PASSED"; tail -30 "$OUT"; exit 1
fi
echo ""
echo "ALL MILESTONES ORG FROM SPACE CHECKS PASSED (on real migrations)"
