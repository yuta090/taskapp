 ...
 */
export async function assertUsersHaveSpaceRole(userIds, spaceId, allowedRoles, fieldLabel) {
    const uniqueIds = Array.from(new Set(userIds));
    if (uniqueIds.length === 0)
        return;
    const supabase = getSupabaseClient();
    ...
    if (mismatched.length > 0) {
        throw new ToolUserError(`${fieldLabel}に指定できるのは、役割が${allowedRoles.join('/')}の人だけです`, 400);
    }
}
/**
