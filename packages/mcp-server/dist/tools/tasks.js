...
export async function taskUpdate(params) {
    ...
    if (params.assigneeInviteId !== undefined) {
        ...
        if (params.assigneeInviteId !== null)
            await assertInvitesAreInSpace([params.assigneeInviteId], params.spaceId);
...
// ユーザー横断でのタスク一覧（新規ツール）
export async function taskListMy(params) {
    const ctx = getAuthContext();
    const supabase = getSupabaseClient();
    ...
    for (const membership of memberships) {
        if (!spaceIds.includes(membership.space_id))
            continue;
        // 権限チェック（read）
        ...
        const spaceData = membership.spaces;
        results.push({
            spaceId: membership.space_id,
            spaceName: spaceData?.name || 'Unknown',
            tasks: (tasks || []).map((t) => withTaskNumber(flattenTaskInternalMetrics(t))),
        });
    }
    return results;
}
