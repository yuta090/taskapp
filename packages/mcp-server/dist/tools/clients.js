...
export async function clientList(params) {
    ...
    if (membersError)
        throw new Error('クライアント一覧の取得に失敗しました: ' + membersError.message);
    let pendingInvites = [];
    if (params.includeInvites) {
        ...
        if (invitesError)
            throw new Error('招待一覧の取得に失敗しました: ' + invitesError.message);
...
export async function clientAddToSpace(params) {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error)
        throw new Error('スペースへの追加に失敗しました: ' + error.message);
    return data;
}
