 ...
 */
function withInviteUrl(invite, reused) {
...
export async function clientList(params) {
    ...
    if (membersError)
        throw hideDbError(membersError, 'client_list', 'クライアント一覧の取得に失敗しました');
    let pendingInvites = [];
    if (params.includeInvites) {
        ...
        if (invitesError)
            throw hideDbError(invitesError, 'client_list (invites)', '招待一覧の取得に失敗しました');
...
export async function clientGet(params) {
    ...
    const { data: membership, error: membershipError } = await supabase
        ...
        .single();
    if (membershipError)
        throw notFoundOr(membershipError, 'client_get', 'クライアントが見つかりません', 'クライアントの取得に失敗しました');
    ...
    if (spacesError)
        throw hideDbError(spacesError, 'client_get (spaces)', 'スペース情報の取得に失敗しました');
    return {
...
export async function clientUpdate(params) {
    ...
    if (params.role !== expectedRole) {
        throw new ToolUserError(`組織での役割（${orgRole}）と合わないロールです。${expectedRole} を指定してください`, 400);
    }
    const { data, error } = await supabase
        .from('space_memberships')
        ...
        .single();
    if (error)
        throw hideDbError(error, 'client_update', 'クライアントの更新に失敗しました');
    return data;
}
export async function clientAddToSpace(params) {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error)
        throw hideDbError(error, 'client_add_to_space', 'スペースへの追加に失敗しました');
    return data;
}
export async function clientInviteList(params) {
    ...
    if (error)
        throw hideDbError(error, 'client_invite_list', '招待一覧の取得に失敗しました');
    return (data || []);
}
