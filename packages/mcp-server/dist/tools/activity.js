...
const OP_TO_ACTION = { I: 'insert', U: 'update', D: 'delete' };
...
async function getOrgId(spaceId) {
    ...
    if (error)
        throw notFoundOr(error, 'activity/getOrgId', 'スペースが見つかりません', 'スペースの取得に失敗しました');
    if (!data)
...
function assertAllowedTable(table) {
    if (!ALLOWED_ACTIVITY_ENTITY_TABLES.has(table)) {
        throw new ToolUserError(`entityTable "${table}" の履歴は見られません（見られる表: ${[...ALLOWED_ACTIVITY_ENTITY_TABLES].join(', ')}）`, 400);
    }
}
function toOp(action) {
    const op = ACTION_TO_OP[action.toLowerCase()];
    if (!op)
        throw new ToolUserError(`action は insert / update / delete のどれかを指定してください（受け取った値: ${action}）`, 400);
    return op;
}
function withAction(rows) {
    return (rows || []).map(row => ({
        ...row,
...
export async function activitySearch(params) {
    ...
    if (params.entityTable)
        query = query.eq('table_name', params.entityTable);
    if (params.entityId)
        query = query.eq('row_pk->>id', params.entityId);
    if (params.actorId)
        query = query.eq('actor_user_id', params.actorId);
    if (op)
...
export async function activityEntityHistory(params) {
    ...
    if (ctx.allowedSpaceIds)
        query = query.in('space_id', ctx.allowedSpaceIds);
    const { data, error } = await query.order('id', { ascending: false }).limit(params.limit);
    if (error) {
