...
async function readMinutes(spaceId, meetingId) {
    ...
    return { orgId, minutesMd: data.minutes_md ?? '' };
}
export async function minutesTaskifyPreview(params) {
    ...
    if (error)
        throw mapRaiseExceptionError(error.message, 'タスク化の候補を取れませんでした');
    const result = data;
    return {
...
export async function minutesTaskify(params) {
    ...
    if (error)
        throw mapRaiseExceptionError(error.message, 'タスク化に失敗しました');
    const result = data;
    return {
        ok: true,
...
//# sourceMappingURL=minutesTaskify.js.map