...
const nullableIdSchema = z.preprocess((v) => (typeof v === 'string' && CLEAR_WORDS.has(v.trim().toLowerCase()) ? null : v), z.string().uuid().nullable().optional());
...
export const wikiUpdateSchema = z.object({
    ...
    expectedUpdatedAt: z
        .string()
        .optional()
        .describe('直前の wiki_get で受け取った updated_at をそのまま渡す。渡すと、その版のままのときだけ書き換える。渡さないと、他の人やAIが先に書いた内容を黙って上書きする'),
});
...
export async function wikiCreate(params) {
    ...
    const body = await toWikiBlocksJson(params.body || '', params.format);
    const { data, error } = await supabase
        .from('wiki_pages')
        .insert({
        org_id: orgId,
...
export async function wikiUpdate(params) {
    ...
    if (params.body !== undefined) {
        updateData.body = await toWikiBlocksJson(params.body, params.format);
    }
...
function removeToc(blocks) {
    if (!blocks.some((b) => b.type === TOC_TYPE))
        return null;
    return blocks.filter((b) => b.type !== TOC_TYPE);
}
export async function wikiToc(params) {
    ...
    const { error: vErr } = await supabase
        .from('wiki_page_versions')
        .insert({ org_id: orgId, page_id: params.pageId, title: data.title, body, created_by: actorId });
    if (vErr)
        console.error('バージョン保存失敗:', vErr.message);
    return data;
}
