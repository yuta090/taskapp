...
export async function minutesCompleteChecked(params) {
    ...
    const minutesMd = meeting.minutes_md ?? '';
...
export const minutesCompleteTools = [
    {
        ...
        description: '議事録でチェックが付いている行（`- [x] … <!--task:uuid-->`）のタスクを完了にする。画面でチェックを入れたときと同じ。すでに完了のものは飛ばすので何度実行してもよい。決まっていない決定事項のタスクは、理由を添えて断る（勝手に決定にはしない）',
        inputSchema: minutesCompleteCheckedSchema,
        handler: minutesCompleteChecked,
    },
];
//# sourceMappingURL=minutesComplete.js.map