...
export const allTools = [...taskTools, ...decisionTools, ...taskImportTools, ...ballTools, ...meetingTools, ...reviewTools, ...milestoneTools, ...spaceTools, ...activityTools, ...clientTools, ...wikiTools, ...minutesTools, ...schedulingTools, ...fileTools];
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        return {
            tools: allTools.map((tool) => ({
                ...
                inputSchema: {
                    type: 'object',
                    properties: Object.fromEntries(Object.entries(tool.inputSchema.shape).map(([key, schema]) => [
                        key,
                        {
...
function getZodType(schema) {
    ...
    switch (typeName) {
        ...
        case 'ZodEnum':
            return 'string';
        case 'ZodOptional':
            return getZodType(schemaAny._def.innerType);
        case 'ZodDefault':
            return getZodType(schemaAny._def.innerType);
        default:
            return 'string';
