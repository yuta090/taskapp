...
async function checkAuth(spaceId, action, toolName, resourceId) {
...
export async function schedulingCreate(params) {
...
export async function schedulingCancel(params) {
...
export async function schedulingGetResponses(params) {
...
function computeAvailableSlots(busyPeriods, options) {
    const { startDate, endDate, durationMinutes, businessHourStart, businessHourEnd } = options;
    const stepMinutes = 30;
    const maxResults = 100;
    if (durationMinutes <= 0 || businessHourStart >= businessHourEnd)
        return [];
...
export async function schedulingSuggestSlots(params) {
    ...
    if (daysDiff < 0)
        throw new Error('startDate は endDate 以前にしてください');
    if (daysDiff > 30)
        throw new Error('最大30日間まで指定できます');
    ...
    const allowedUserIds = (spaceMembers || []).map((m) => m.user_id);
    ...
    const results = await Promise.allSettled(typedConnections.map(async (conn) => {
        const token = await getValidAccessToken(conn);
        if (!token)
            throw new Error(`Token unavailable for user ${conn.owner_id}`);
...
export async function schedulingSendReminder(params) {
