 ...
 */
export function translateDryRunBusinessError(error) {
    if (!error)
        return undefined;
...
export function mapConfirmProposalError(data) {
    switch (data?.error) {
        ...
        case 'proposal_not_open':
            return new ToolUserError(`この提案は現在「${proposalStatusLabel(data.current_status ?? '')}」のため確定できません`, 409);
        case 'slot_not_found':
