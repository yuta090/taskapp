 ...
 */
export async function assertInSpace(
  table: string,
  id: string,
  spaceId: string,
  notFoundMessage = '指定されたIDが見つからないか、このプロジェクトのものではありません'
): Promise<void> {
  ...
  if (error) throw new Error(`確認に失敗しました（${table}）`)
  if (!data) throw new ToolUserError(notFoundMessage, 404)
}
 ...
 */
export async function assertUsersInSpaceOrg(
  userIds: string[],
  spaceId: string
  ...
  const { data: memberships, error: memberError } = await supabase
    ...
    .select('user_id, role')
    .eq('org_id', (space as { org_id: string }).org_id)
    .in('user_id', uniqueIds)

  if (memberError) throw new Error('メンバーの確認に失敗しました')

  const byUser = new Map(
    ((memberships || []) as Array<{ user_id: string; role: string }>).map((m) => [
      m.user_id,
      { role: m.role },
    ])
  )
...
export function requireActorUserId(): string {
  ...
  if (!userId) {
    throw new ToolUserError(
      'この鍵には利用者が紐づいていないため実行できません。個人の鍵を使ってください',
      400
    )
  }
  return userId
