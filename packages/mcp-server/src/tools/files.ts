...
export async function fileList(params: z.infer<typeof fileListSchema>): Promise<FileListItem[]> {
  ...
  if (error) throw new Error('ファイル一覧の取得に失敗しました')
  return ((data || []) as Record<string, unknown>[])
    .map((f) => ({
      ...
      link: buildFileDownloadLink(f.id as string),
      downloadPath: buildFileDownloadLink(f.id as string),
    }))
}

export async function fileUploadUrl(params: z.infer<typeof fileUploadUrlSchema>): Promise<FileUploadUrlResult> {
...
export async function fileUploadComplete(
  params: z.infer<typeof fileUploadCompleteSchema>,
): Promise<FileUploadCompleteResult> {
  ...
  if (error || !file) throw new Error('ファイルが見つかりません')

  if (!ctx.userId || file.uploaded_by !== ctx.userId) {
    throw new Error('権限エラー: 自分がアップロードしたファイルだけ完了にできます')
  }

  const downloadPath = buildFileDownloadLink(file.id as string)
  const tablePath = isTabularName(file.name as string)
    ? `/${file.org_id as string}/project/${file.space_id as string}/files/${file.id as string}`
    : null

  // 二重実行しても副作用が増えないよう、既に ready なら何もしない
  if (file.status === 'ready') {
    return { ok: true, fileId: file.id as string, name: file.name as string, downloadPath, tablePath, message: 'すでにアップロード済みです' }
  }
  ...
  if (listError) throw new Error('アップロードの確認に失敗しました')
  const exists = (listing || []).some((entry: { name: string }) => entry.name === fileName)
  if (!exists) throw new Error('アップロードが完了していません（Storage にファイルがありません）')

  const { error: updateError } = await supabase.from('files').update({ status: 'ready' }).eq('id', params.fileId)
  if (updateError) throw new Error('ファイルの完了処理に失敗しました')
...
export const fileTools = [
  ...
  },
  {
    ...
    description:
      'ファイルのアップロードを開始する。files に pending 行を作り、Storage への署名アップロードURLを返す。実バイトは署名URLへ PUT し、その後 file_upload_complete を呼ぶ。spaceId・name・sizeBytes 必須（50MB まで）',
