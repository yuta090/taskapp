...
const DECISION_STATES = ['considering', 'decided', 'implemented'] as const
...
export async function specDecide(
  params: z.infer<typeof specDecideSchema>
): Promise<SpecDecideResult> {
  ...
  const { data: task, error: taskError } = await supabase
    ...
    .single()
  if (taskError || !task) throw new Error('タスクが見つかりません')
  ...
  if (row.type !== 'spec') {
    throw new Error(
      'このタスクは「決定事項のタスク」ではありません。Wiki ページ（仕様書として扱う）を紐づけると決定事項のタスクになります'
    )
  }
  if (
    (params.state === 'decided' || params.state === 'implemented') &&
  ...
  if (error) throw mapRaiseExceptionError(error.message, '決定の記録に失敗しました')
...
export const decisionTools = [
  {
    name: 'spec_decide',
    description:
      '決定事項のタスクを確定する（画面の「決定にする」と同じ）。決定すると紐づく Wiki ページに決定行が入り、確定時点の本文が控えとして残る',
