...
// ---- 見出しの別名（英語キー / 日本語ラベル。書き出しCSV(api/export/tasks)のラベルと揃える） ----

type Column =
  | 'title' | 'description' | 'status' | 'ball' | 'origin' | 'client_scope'
  | 'start_date' | 'due_date' | 'assignee' | 'client_owners' | 'internal_owners'
  | 'parent' | 'priority' | 'milestone'
...
function lookupAlias<T extends string>(table: Record<string, T>, raw: string): T | undefined {
  const key = normalizeKey(raw)
  // 英語キーは小文字化した表で、日本語はそのまま照合する
  for (const [alias, value] of Object.entries(table)) {
    if (normalizeKey(alias) === key) return value
  }
  return undefined
}
...
export function parseTaskImportCsv(text: string): ParsedImport {
  ...
  for (const rec of body) {
    ...
    if (title === '') fail('title（タイトル）が空です')

    const enumField = <T extends string>(col: Column, table: Record<string, T>): T | null => {
      const raw = get(col)
      if (raw === '' || raw === '-') return null
      const v = lookupAlias(table, raw)
      if (v === undefined) fail(`${col} の値「${raw}」は使えません（${[...new Set(Object.values(table))].join(' | ')}）`)
      return v ?? null
    }
    const dateField = (col: Column): string | null => {
      const raw = get(col)
      if (raw === '' || raw === '-') return null
      const v = normalizeDate(raw)
      if (v === null) fail(`${col} の値「${raw}」は日付として読めません（YYYY-MM-DD）`)
      return v
    }
    ...
    if (startDate && dueDate && startDate > dueDate) fail(`start_date(${startDate}) が due_date(${dueDate}) より後です`)

    let priority: number | null = null
    const rawPriority = get('priority')
    if (rawPriority !== '' && rawPriority !== '-') {
      const n = Number(rawPriority)
      if (!Number.isInteger(n) || n < 0 || n > 3) fail(`priority の値「${rawPriority}」は 0〜3 の整数にしてください`)
      else priority = n
    }
    ...
    if (rowErrors.length > 0) {
      for (const message of rowErrors) errors.push({ line: rec.line, title: title || undefined, message })
      continue
    }
...
function resolveUser(ref: string, users: UserDirectory): { id: string } | { error: string } {
  ...
  if (ids.length === 0) return { error: `担当者「${trimmed}」はこのプロジェクトのメンバーに見つかりません（メールアドレスでも指定できます）` }
  if (ids.length > 1) return { error: `担当者「${trimmed}」と同じ表示名のメンバーが複数います。メールアドレスで指定してください` }
  return { id: ids[0] }
}

export function planTaskImport(input: PlanInput): ImportPlan {
  const errors: ImportIssue[] = []
  const skipped: ImportPlan['skipped'] = []
  const errorLines = new Set<number>()
  const fail = (row: ImportRow, message: string) => {
  ...
  const active: ImportRow[] = [] // 作成対象として残った行
  ...
  for (const row of active) {
    ...
    if (row.assignee) {
      ...
      if ('error' in r) fail(row, r.error)
      else assigneeId = r.id
    }
    ...
    if (row.milestone) {
      ...
      if (!id) fail(row, `マイルストーン「${row.milestone}」がこのスペースにありません`)
      else milestoneId = id
    }
  ...
  const autoParents = new Map<string, { id: string; anyDeliverable: boolean }>()
  type ParentRef = { kind: 'row'; row: ImportRow } | { kind: 'existing'; id: string } | { kind: 'auto'; title: string }
  const parentOf = new Map<number, ParentRef>()
  ...
  for (const row of active) {
    ...
    if (!autoParents.has(row.parent)) autoParents.set(row.parent, { id: input.newId(), anyDeliverable: false })
    if (row.clientScope === 'deliverable') autoParents.get(row.parent)!.anyDeliverable = true
    parentOf.set(row.line, { kind: 'auto', title: row.parent })
  }

  // 4) 深さと循環。CSV内の親の連鎖をたどる
  const depthOf = new Map<number, number>()
  const depthFor = (row: ImportRow, trail: Set<number>): number | 'cycle' => {
    if (depthOf.has(row.line)) return depthOf.get(row.line)!
    const ref = parentOf.get(row.line)
    let d: number
    if (!ref) d = 0
    else if (ref.kind === 'existing') d = 1 // 既存側の深さは分からない。DBトリガーが最終防衛
    else if (ref.kind === 'auto') d = 1
    else {
      if (trail.has(ref.row.line)) return 'cycle'
      trail.add(row.line)
      const pd = depthFor(ref.row, trail)
      if (pd === 'cycle') return 'cycle'
      d = pd + 1
    }
  ...
  for (const row of active) ids.set(row.line, input.newId())
  ...
  for (const row of active) {
    ...
    if (ref?.kind === 'row') parentId = ids.get(ref.row.line)!
    else if (ref?.kind === 'existing') parentId = ref.id
    else if (ref?.kind === 'auto') parentId = autoParents.get(ref.title)!.id
    tasks.push({
      id: ids.get(row.line)!,
      line: row.line,
      title: row.title,
