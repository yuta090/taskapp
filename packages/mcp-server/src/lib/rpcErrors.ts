...
interface RaisePattern {
  ...
  toMessage: (match: RegExpMatchArray) => string
}

const RAISE_PATTERNS: RaisePattern[] = [
  ...
  },
  {
    ...
    status: 403,
    toMessage: () =>
      'レビューを取り消せるのは、依頼した本人・プロジェクトの管理者・組織のオーナーだけです',
  },
 ...
 */
export function mapRaiseExceptionError(message: string, fallbackMessage: string): Error {
  for (const pattern of RAISE_PATTERNS) {
    const match = message.match(pattern.test)
    if (match) return new ToolUserError(pattern.toMessage(match), pattern.status)
  }
