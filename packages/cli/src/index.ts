...
import { showNewNotices } from './notices.js'
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      if (showNewNotices(manifest) === 0) console.log(chalk.gray('新しいお知らせはありません'))
    } catch (e) {
...
async function main() {
  ...
  if (apiUrl) {
    ...
    // まだ見ていないお知らせを 1 回だけ stderr に出す(--json の stdout は汚さない。機械実行では静かにしておく)
    if (!process.argv.includes('--json')) showNewNotices(manifest)
  } else {
