...
- **worktree の置き場所は `../wt/` に集約する（厳守）**。リポジトリの兄弟に散らかすと `scripts/` 直下が worktree だらけになるため、**`/Volumes/WIN-MAC2/scripts/wt/` の中**にまとめる。`scripts/` には他プロジェクトもあるので、名前は `taskapp-<topic>` とプロジェクト名で始める。
- **worktree の作成**:
  ```bash
  # セキュリティ作業を隔離する例（メイン作業ディレクトリはそのまま機能開発に使える）
  git worktree add -b security/<topic> ../wt/taskapp-<topic> origin/develop
  git -C ../wt/taskapp-<topic> <commit/push など>   # cd せず -C で操作すると安全
  ```
  リポジトリ内には作らない。`../taskapp-wt-<topic>`（旧方式・兄弟ディレクトリ直置き）は使わない。
- **後片付け**: マージ後に `git worktree remove ../wt/taskapp-<topic>` で削除。放置しない。
